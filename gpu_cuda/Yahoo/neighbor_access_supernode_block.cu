    ...
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
  ...
  // printf("here n: %d\n",n);
        ...
        if (extraCount > 0) {
            ...
            multiplesCount = readGamma(GAMMA, arr, &tmp_s);
                printf("multiplesCount:%d",multiplesCount);

            if (multiplesCount != 0) {
                multOffset = tmp_s;
                //跳过重边列表    (值可不读，len可能得读
                for (int i = 0; i < multiplesCount; i++) {
                    readGamma(GAMMA, arr, &tmp_s);
 ...
 printf("intervalCount:%d",extraCount);
            if (intervalCount != 0) {
        ...
        while (index < d) {
              if(flag==1&&mlen>0) mlen--; 
              else
             // if(currInterval==MAX_LABEL && currMult== MAX_LABEL) printf("从哪开始：%d,%d\n",result,index);
    ...
    ////__syncthreads();
      /* int n = off + blockDim.x * blockIdx.x + threadIdx.x;
       if (n > MAX_LABEL) return;
        ...
        travel(NodeOffset, 0, dv2, GAMMA, ZETA_3, 234, arr, s, d_Timestamps, t_pos,
...
__global__ void travel_234(int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
    if (x >= MAX_LABEL) return;
      int tid = threadIdx.x;
      int label = (threadIdx.x < blockDim.x - 32) ? 0 : 1;
       cg::coalesced_group block = cg::coalesced_threads();

    auto producer_group = cg::binary_partition(block, block.thread_rank() < 480);
    __shared__ struct {
        // 时间戳缓存
        uint32_t timestamp_cache[512];
        int curr_pos;//当前取到哪
        int rear_pos;//存到哪儿
        uint64_t timestamp_current_bit;//这一轮解码开始位置
        uint64_t timestamp_next_bit;
        
    } smem; 
    __shared__ uint64_t smem_cursor;
    __shared__ uint32_t smem_previous;
    __shared__ int smem_idx;
    __shared__ uint64_t decoded_ends[512];
    __shared__ uint32_t decoded_vals[512];

    

    if (tid == 0) {
        smem_cursor = smem.timestamp_current_bit = getOffset(x, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
        smem_previous = minTimestamp;
        smem.timestamp_next_bit = getOffset(x + 1, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
        smem_idx = 0;
    }
    block.sync();
        uint64_t end_bit = smem.timestamp_next_bit;
          
        if (producer_group.thread_rank() < producer_group.size()){
            while (smem.timestamp_current_bit < end_bit) {
                // 并行解码 32 个候选时间戳
                uint64_t my_end;
                uint64_t my_start = my_end = smem.timestamp_current_bit + producer_group.thread_rank();
                if (my_start >= end_bit) break;

                uint32_t my_val = readZeta_k2(d_Timestamps, &my_end);   // readZeta_k2
                if(my_end==my_start) printf("stop\n");
                decoded_vals[producer_group.thread_rank()] = my_val;
                decoded_ends[producer_group.thread_rank()] = my_end;

                producer_group.sync();

                if (producer_group.thread_rank() == 0) {
                    int curr_tid = 0;
                    uint64_t start = smem.timestamp_current_bit;
                    uint64_t cursor = start;
                    int idx = 0;
                    int previous = smem_previous;
                    while (curr_tid <producer_group.size()) {
                        // 当前线程的结束位置
                        uint64_t curr_end = decoded_ends[curr_tid];
                        if (curr_end > end_bit) break;
                            uint32_t ts = previous + inv_v_func(decoded_vals[curr_tid]);
                            int curr_rear =  smem.rear_pos;
                            int new_rear =  (curr_rear + 1 )%512;
                            
                            while ( new_rear == smem.curr_pos) {
                                  __threadfence_block();
                            }
                            smem.timestamp_cache[curr_rear] = ts;
                            __threadfence_block();
                            previous = ts;
                            smem.rear_pos = new_rear;
                            cursor = curr_end;

                            // 下一合法线程的 tid:
                            curr_tid = cursor - start;
                         
                    } 
                    smem_previous = previous;
                     // ✅ 插入这个安全防御逻辑：放在更新 cursor 的地方
                     
                    smem.timestamp_current_bit = cursor;

                    if (cursor == start) {
                              // 防止死循环
                              printf("cursor==start\n");
                    smem.timestamp_current_bit = end_bit;  // 退出外层 while 循环
                      // 消费者看到无可用数据，安全返回
                    }  
                }
                 producer_group.sync();
    ...
    if (tid == blockDim.x - 1) {
      ...
      // printf("here n: %d\n",n);
            ...
            while (index < d) {
                ...
                int idx = smem.curr_pos;
                while (idx == smem.rear_pos ) {
                    if (smem.timestamp_current_bit >= smem.timestamp_next_bit) break;
                      __threadfence_block();
                          idx = smem.curr_pos;  // 重新加载最新值
                      }

                  if (idx !=smem.rear_pos) {
                      uint32_t t = smem.timestamp_cache[idx];
                   //   if(t!=timestamp) printf("x:%d, timestamp:%d,t:%d\n",x,timestamp,t);
                      smem.curr_pos = (idx + 1) % 512;
                      index++;
                      }
...
cudaMemset(d_task_index, 0, sizeof(int));
  ...
  travel_234<<<1,512>>>( 0, dv2, GAMMA, ZETA_3, 234,Graph, g_pos, d_Timestamps, t_pos,
               d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);

   // travelGraph << < 1,1, 0 >> > (off, dv2,d_task_index, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
