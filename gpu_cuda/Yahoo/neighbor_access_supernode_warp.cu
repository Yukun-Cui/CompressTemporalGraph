    ...
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
    ...
    if (warp_id == 1) {
        ...
        if (lane_id == 0) {
            ...
            smem.timestamp_current_bit = getOffset(x, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
            smem.timestamp_next_bit = getOffset(x + 1, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
            smem.curr_pos = 0;
            smem.rear_pos = 0;
            smem.only0_count=0;
            smem.previous = minTimestamp;
        }
    }
    ...
    bool cond;
        while (true) {
            // 并行解码 32 个候选时间戳
             uint64_t my_end;
            uint64_t my_start = my_end = smem.timestamp_current_bit + lane_id;
            
            uint32_t my_val = readZeta_k2(d_Timestamps, &my_end);   // readZeta_k2

            uint64_t cursor = smem.timestamp_current_bit;
            int write_index = 0;
            int decoded_count = 0;

            while (write_index < 32) {
                bool is_valid = (my_start == cursor) && (my_end > my_start);
                unsigned mask = __ballot_sync(0xffffffff, is_valid);
                if (mask == 0) break;
              //  if (lane_id == 0 && mask == 0x1) {
                //    smem.only0_count++;
                   // }

                int lane = __ffs(mask) - 1;
                uint32_t val = __shfl_sync(0xffffffff, my_val, lane);
                size_t end = __shfl_sync(0xffffffff, my_end, lane);


                if (lane_id == 0) {
                    smem.timestamp_cache[decoded_count++] = smem.previous = (int)(inv_v_func(val) + smem.previous);//previous = minTimestamp
                    cursor = end;
                    // decoded_count++;
                }
                __syncwarp();
            ...
            if (lane_id == 0) {
                ...
                smem.curr_pos = 0;
                smem.rear_pos = decoded_count;
            }

            __syncwarp();

            // 等待邻居 warp 消耗完缓存
            while (smem.curr_pos < smem.rear_pos) {
                __nanosleep(10);
            }
            ...
            if (lane_id == 0) {
            cond = smem.timestamp_current_bit >= smem.timestamp_next_bit;
            }
            cond = __shfl_sync(0xffffffff, cond, 0);  // 所有线程从 lane0 获知判断
            if (cond) break;
        }
       if(lane_id==0)
        printf("只有0线程的次数：%d\n", smem.only0_count);
      ...
      // printf("here n: %d\n",n);
            ...
            while (index < d) {
              ...
              //  previous = timestamp;
                ...
                uint32_t t = smem.timestamp_cache[smem.curr_pos++];
...
cudaMemset(d_task_index, 0, sizeof(int));
  ...
  travel_234<<<1,64>>>( 0, dv2, GAMMA, ZETA_3, 234,Graph, g_pos, d_Timestamps, t_pos,
               d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
