...
__device__ __forceinline__ uint64_t readUnary(uint8_t* arr, uint64_t* s) {
    ...
    while (true) {
        uint8_t byte = __ldg(&arr[byteIdx]);
...
*/
__device__ uint64_t readGamma(int* GAMMA, uint8_t* arr, uint64_t* s) {
    uint64_t byteIndex = *s >> 3;
    uint8_t bitOffset = *s & 7;

   
    uint8_t b0 = __ldg(&arr[byteIndex]);
    uint8_t b1 = __ldg(&arr[byteIndex + 1]);
    uint8_t b2 = __ldg(&arr[byteIndex + 2]);
    ...
    int preComp;
      if ((preComp = __ldg(&GAMMA[raw & 0xFFFF])) != 0) {
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
    ...
    for (uint64_t p = 0; p < l; p++) {
        ...
        uint8_t byte = __ldg(&arr[bit_pos >> 3]);
...
__device__ uint64_t readZeta_k(int* ZETA_3, uint8_t* arr, uint64_t* s) {
    // 位操作准备
    uint64_t byteIndex = *s >>3;
    uint8_t bitOffset = *s &7;

    // 向量化三字节提取
    uint32_t data = ((uint32_t)__ldg(&arr[byteIndex]) << 16) |
                    ((uint32_t)__ldg(&arr[byteIndex + 1]) << 8) |
                    ((uint32_t)__ldg(&arr[byteIndex + 2]));
    data <<= bitOffset;
    uint16_t result = data >> 8;  // 提取高 16 位

    // 查表预解码
    int preComp = __ldg(&ZETA_3[result & 0xFFFF]);
    if (preComp != 0) {
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
    ...
    if (val < left) {
        *s = pos + len - 1;
        ...
        uint64_t lastBit = (__ldg(&arr[(pos + len - 1) >>3]) >> (7 - ((pos + len - 1) &7))) & 1;
        *s = pos + len;
...
// 引用链邻居解码：用显式状态栈统一模拟 nextRefValue/ref_nextValue 的调用过程。
int __device__ ref_nextValue(int* GAMMA, int* ZETA_3, uint8_t* arr, NodeState* refChain, int ind) {

    enum FuncType { NEXT_REF_VALUE, REF_NEXT_VALUE };
    enum State {
        NRV_INIT,
        NRV_PROCESS_VALUE,
        NRV_PROCESS_REF,
        ...
        RNV_INIT,
        RNV_AFTER_REF_CALL,
        RNV_AFTER_VALUE_CALL
    };
    enum StepType {
        STEP_NONE,
        STEP_SKIP_FIRST_BLOCK,   // 第一次跳过无效引用块（不保存结果）
        STEP_FETCH_VALUE,        // 真正取值（需要保存结果）
        STEP_SKIP_NEXT_BLOCK     // 跳过下一个块中的无用值
    };
    typedef struct {
        FuncType func;
        State state;
        int ind;
        int return_to;      // index in stack
        int result;         // store result
        int step;           // block step
        int value_ref;
        int value_interval;
        StepType step_type = STEP_NONE;
    } Frame;
    Frame stack[32];
    int top = 0;
    
    Frame f = { REF_NEXT_VALUE, RNV_INIT, ind, -1, 0, 0, 0, 0};
    stack[0] = f;
    int final_result = 0;  // 存储最终结果
     
    while (top >= 0) {
        Frame& f = stack[top];
        int current_result = 0;  // 当前帧的临时结果

        if (f.func == REF_NEXT_VALUE) {
            switch (f.state) {
            case RNV_INIT:
                if (refChain[f.ind].ref_flag == 0) {
                    // 压入 nextRefValue 调用
                    stack[++top] = { NEXT_REF_VALUE, NRV_INIT, f.ind, top - 1,MAX_LABEL, 0, 0, 0,f.step_type };//继承f的step_type?
                    f.state = RNV_AFTER_REF_CALL;
                }
                else if (refChain[f.ind].ref_flag == 1) {
                    // ref already known  
                    stack[++top] = { NEXT_REF_VALUE, NRV_INIT, f.ind, top - 1, MAX_LABEL, 0, 0, 0,f.step_type };//继承f的step_type?
                     
                     
                    if (refChain[f.ind].flag == 1) f.value_interval = refChain[f.ind].currMult;
                    else if (refChain[f.ind].flag == 2) f.value_interval = refChain[f.ind].currInterval;
                    else if (refChain[f.ind].flag == 3) f.value_interval = refChain[f.ind].currResidual;
                    f.state = RNV_AFTER_REF_CALL;
                }
                else if (refChain[f.ind].ref_flag == 2) {
                    f.value_interval = nextValue(GAMMA, ZETA_3, refChain[f.ind].x, arr, &refChain[f.ind]);
                    f.value_ref = refChain[f.ind].pre_refvalue;
                    f.state = RNV_AFTER_VALUE_CALL;
                }
                break;

            case RNV_AFTER_REF_CALL:
                // 获取子调用结果
                f.value_ref = f.result;
                if (refChain[f.ind].ref_flag == 0) {
                    f.value_interval = nextValue(GAMMA, ZETA_3, refChain[f.ind].x, arr, &refChain[f.ind]);
                    f.state = RNV_AFTER_VALUE_CALL;
                    refChain[f.ind].pre_refvalue = f.value_ref;
                }
                else {
                    current_result = (f.value_ref < f.value_interval) ? f.value_ref : f.value_interval;
                    refChain[f.ind].ref_flag = (f.value_ref < f.value_interval) ? 1 : 2;
                    refChain[f.ind].pre_refvalue = f.value_ref;
                    // 返回结果给调用者
                    f.result = current_result;
                    if (f.return_to >= 0) {
                        stack[f.return_to].result = current_result;
                    }
                    top--;
                }
                break;

            case RNV_AFTER_VALUE_CALL: 
                current_result = (f.value_ref < f.value_interval) ? f.value_ref : f.value_interval;
                refChain[f.ind].ref_flag = (f.value_ref < f.value_interval) ? 1 : 2;
                refChain[f.ind].pre_refvalue = f.value_ref;
                // 返回结果给调用者
        ...
        else if (f.func == NEXT_REF_VALUE) {
            switch (f.state) {
            case NRV_INIT:
                if (refChain[f.ind].blockNum >= refChain[f.ind].blockCount) {
                    current_result = MAX_LABEL;
                     f.result = current_result;  
                    if (f.return_to >= 0) {
                        stack[f.return_to].result = current_result;
                    }
                ...
                if (refChain[f.ind + 1].ref == 0) {
                    f.state = NRV_PROCESS_VALUE;
                }
                else {
                    f.state = NRV_PROCESS_REF;
                }
                break;

            case NRV_PROCESS_VALUE:
                if (refChain[f.ind].currblock == 0 && refChain[f.ind].ref_flag == 0) {
                    refChain[f.ind].blockNum++;
                    if (refChain[f.ind].blockNum == refChain[f.ind].blockCount - 1) {
                        refChain[f.ind].blockNum++;
                    }

                    refChain[f.ind].currblock = readGamma(GAMMA, arr, &refChain[f.ind].refOffset) + 1;

                    // 处理整个块
                    for (int i = 0; i < refChain[f.ind].currblock; i++) {
                        nextValue(GAMMA, ZETA_3, refChain[f.ind + 1].x, arr, &refChain[f.ind + 1]);
                    }

                    refChain[f.ind].blockNum++;
                    refChain[f.ind].currblock = (refChain[f.ind].blockNum == refChain[f.ind].blockCount - 1)
                        ? refChain[f.ind + 1].d - refChain[f.ind].cpt
                        : readGamma(GAMMA, arr, &refChain[f.ind].refOffset) + 1;
                }

                // 处理当前值
                current_result = nextValue(GAMMA, ZETA_3, refChain[f.ind + 1].x, arr, &refChain[f.ind + 1]);
                refChain[f.ind].currblock--;

                if (refChain[f.ind].currblock == 0) {
                    refChain[f.ind].blockNum++;
                    if (refChain[f.ind].blockNum == refChain[f.ind].blockCount) {
                        refChain[f.ind].blockNum++;
                    }
                    ...
                    refChain[f.ind].blockNum++;
                    if (refChain[f.ind].blockNum == refChain[f.ind].blockCount) {
                        refChain[f.ind].currblock = refChain[f.ind + 1].d - refChain[f.ind].cpt;
                    }
                    else {
                        refChain[f.ind].currblock = readGamma(GAMMA, arr, &refChain[f.ind].refOffset) + 1;
                    }
            ...
            case NRV_PROCESS_REF:
                if (refChain[f.ind].currblock == 0) {
                    ...
                    if (refChain[f.ind].currblock > 0) {
                        f.step = 0;
                        f.step_type = STEP_SKIP_FIRST_BLOCK;
                        f.state = NRV_WAIT_REF_RESULT;
                        stack[++top] = { REF_NEXT_VALUE, RNV_INIT, f.ind + 1, top - 1, 0, 0, 0, 0, STEP_SKIP_FIRST_BLOCK };
                        break;
                    }
                }
                ...
                // 第二次：真正取值
                f.step = 0;
                f.step_type = STEP_FETCH_VALUE;
                f.state = NRV_WAIT_REF_RESULT;
                stack[++top] = { REF_NEXT_VALUE, RNV_INIT, f.ind + 1, top - 1, 0, 0, 0, 0, STEP_FETCH_VALUE };
                break;

            case NRV_WAIT_REF_RESULT:
                if (f.step_type == STEP_SKIP_FIRST_BLOCK || f.step_type == STEP_SKIP_NEXT_BLOCK) {
                    f.step++;
                    if (f.step < refChain[f.ind].currblock) {
                        // 继续跳过
                        stack[++top] = { REF_NEXT_VALUE, RNV_INIT, f.ind + 1, top - 1, 0, f.step, 0, 0, f.step_type };
                    }
                    else {
                        // 跳过完毕，进入下一阶段
                        if (f.step_type == STEP_SKIP_FIRST_BLOCK) {
                            // 再次更新块号，准备正式取值
                            refChain[f.ind].blockNum++;
                            if (refChain[f.ind].blockNum == refChain[f.ind].blockCount - 1) {
                                refChain[f.ind].currblock = refChain[f.ind + 1].d - refChain[f.ind].cpt;
                            ...
                            if (refChain[f.ind].blockNum >= refChain[f.ind].blockCount) {
                                // 所有块完成
                                if (f.return_to >= 0) stack[f.return_to].result = f.result;
                                top--;
                            }
                            else {
                                refChain[f.ind].currblock = (refChain[f.ind].blockNum == refChain[f.ind].blockCount)
                                    ? refChain[f.ind + 1].d - refChain[f.ind].cpt
                                    : readGamma(GAMMA, arr, &refChain[f.ind].refOffset) + 1;
                                f.step = 0;
                                f.step_type = STEP_FETCH_VALUE;
                ...
                if (f.step_type == STEP_FETCH_VALUE) {
                    // 真正获取值
                    f.result = current_result;
                    refChain[f.ind].currblock--;
                     if (refChain[f.ind].currblock > 0) {
                        f.step++;  // 可选：用于调试跟踪第几个
                        stack[++top] = { REF_NEXT_VALUE, RNV_INIT, f.ind + 1, top - 1, 0, f.step, 0, 0, STEP_FETCH_VALUE };
                        break;
                    }
                    if (refChain[f.ind].currblock == 0) {
                        // 第三个阶段：跳过无效块中的值
                        ...
                        if (refChain[f.ind].blockNum == refChain[f.ind].blockCount) {
                            refChain[f.ind].blockNum++;  // 跳过无效块
                        }
                        refChain[f.ind].currblock = readGamma(GAMMA, arr, &refChain[f.ind].refOffset) + 1;
                        f.step = 0;
                        f.step_type = STEP_SKIP_NEXT_BLOCK;
                        f.state = NRV_WAIT_REF_RESULT;
                        stack[++top] = { REF_NEXT_VALUE, RNV_INIT, f.ind + 1, top - 1, 0, 0, 0, 0  };
                    }
                    if (f.return_to >= 0)
                        stack[f.return_to].result = f.result;
        ...
        // 更新当前帧的结果（如果需要）
        if (f.func == NEXT_REF_VALUE || f.func == REF_NEXT_VALUE) {
            f.result = current_result;
        }
    }
...
__device__ void get_dest(  int off ,int dest_size, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc)
{
  ...
  // printf("get dest_size: %d\n", dest_size);
   ...
   // printf("here berfore travel\n");
    ...
    while (index < d) {
        result = ref_nextValue(GAMMA, ZETA_3, arr, refChain, ind);
...
__device__ void travel( int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
    ...
    // if (n > 33) return;
      ...
      //*t_pos = 83395934;
       //节点6586：
    // *s = get_position(GAMMA, arr, d_Offsets, x);
   //  if (n == 31) printf("position:%lu", *s);
    // *t_pos = T_offset[x];
  //  uint64_t tmp_s = off>=0?NodeOffset[off] : getOffset(x, 7, d_lowerBitsVector, d_selectUpper); 
    
	uint64_t tmp_s = getOffset(x, gef_low_len, d_lowerBitsVector, d_selectUpper);
	 
    uint64_t tmp_t = getOffset(x, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
       

  //  uint64_t tmp_t = getOffset(x, 6, d_t_lowerBitsVector, d_t_selectUpper);
    // printf("错了错了\n");


    int d = readGamma(GAMMA, arr, &tmp_s);
    if (d < 4) dv2[x] = true;
    // printf("d:%d\n", d);
    if (d == 0) {
        // dv2[x] = true;
  ...
  // printf("here n: %d\n",n);
        ...
        while (index < d) {
           //  result = nextValue(GAMMA, ZETA_3, x, arr, Node);
              int result = nextValue(GAMMA, ZETA_3, x, arr,
         ...
         //   if (*t_pos > 13701410 * 8) {
           //     printf("out of range\n" );
            ...
            int timestamp = (int)(inv_v_func(readZeta_k2(d_Timestamps, &tmp_t)) + previous);
...
__device__ void travel2( int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
     if (x >= MAX_LABEL) return;
    //curand_init(2, n, 0, &state);
  ...
  //  uint64_t tmp_t = getOffset(x1, 6, d_t_lowerBitsVector, d_t_selectUpper);
    ...
    //  tmp_s = *s;
	  ...
	  if(ref==0) return;
           ...
           get_dest( off,d, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1);
    ...
    ////__syncthreads();
       ...
       if (n > MAX_LABEL  ) return;
        ...
        // 处理节点 nodeIdx 的邻居
         ...
         travel( 0, dv2, GAMMA, ZETA_3, n  , arr,  s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
...
__global__ void travelGraph2(int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
    ...
    ////__syncthreads();
       int n = off + blockDim.x * blockIdx.x + threadIdx.x;
       if (n > MAX_LABEL*8) return;
       
        // 处理节点 nodeIdx 的邻居
        if(n%8==0)
         travel2( 0, dv2, GAMMA, ZETA_3, n/8, arr,  s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    //__shared__ int currentNodeIdx; // 使用共享内存存储当前处理的节点索引
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    travelGraph << < grid_size , 512, 0 >> > (off, dv2, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);


    travelGraph2 << < grid_size*8, 512, 0 >> > (off, dv2, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
