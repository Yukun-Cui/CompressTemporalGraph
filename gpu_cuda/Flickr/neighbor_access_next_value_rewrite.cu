...
__device__ uint64_t readZeta_k(int* ZETA_3, uint8_t* arr, uint64_t* s) {
    ...
    // 查表预解码 
    int preComp = ZETA_3[result & 0xFFFF];
    if (preComp != 0) {
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
...
int __device__ nextValue_opt(int* GAMMA, int* ZETA_3, int& x, uint8_t* arr,
...
__device__ int nextMult_opt(int* GAMMA, int& x, uint8_t* arr, 
    int& currMult, int& len, int& prev, uint64_t* multOffset, bool advance)
{
    if (*multOffset == 0) {
        currMult = MAX_LABEL;
        return currMult;
...
__device__ int nextInterval_opt(int* GAMMA, int& x, uint8_t* arr, 
    int& currInterval, int& len, int& prev, uint64_t* IOffset, bool advance)
{
    if (*IOffset == 0) {
        currInterval = MAX_LABEL;
        return currInterval;
...
__device__ int nextResidual_opt(int* ZETA_3, int& x, uint8_t* arr, 
    int& currResidual, int& prev, uint64_t* residualOffset, bool advance)
{
    if (*residualOffset == 0) {
        currResidual = MAX_LABEL;
        return currResidual;
...
int min3_with_flag(int a, int b, int c, int& flag_out) {
    if (a <= b && a <= c) {
        flag_out = 1;
        return a;
    }
...
__device__ int nextValue(
    int* GAMMA, int* ZETA_3, int& x, uint8_t* arr,
    int& currMult, int& mlen, int& m_prev, uint64_t* multOffset, int& multiplesCount, int& multiplesNum,
    int& currInterval, int& Ilen, int& I_prev, uint64_t* IOffset, int& intervalCount, int& intervalNum,
    int& currResidual, int& r_prev, uint64_t* residualOffset, int& residualCount, int& residualNum, int& flag)
{
    // 全部读完，返回 MAX_LABEL
    if (multiplesNum >= multiplesCount && intervalNum >= intervalCount && residualNum >= residualCount) {
        currResidual = currInterval = currMult = MAX_LABEL;
        flag = 3;
        return MAX_LABEL;
    }

    int multVal = MAX_LABEL, intervalVal = MAX_LABEL, residualVal = MAX_LABEL;
    bool advanceMult = false, advanceInterval = false, advanceResidual = false;

    if (flag == 0) {
        // 第一次访问，三个区间都需要 advance（推进状态）
        advanceMult = advanceInterval = advanceResidual = true;
    }
    else if (flag == 1) {
        if (mlen != 0) {
            mlen--;
        ...
        advanceResidual = true;
    }

    // 对所有区间统一调用（是否推进由上面控制）
    multVal = (multiplesNum < multiplesCount) ?
        nextMult_opt(GAMMA, x, arr, currMult, mlen, m_prev, multOffset, advanceMult) : MAX_LABEL;

    intervalVal = (intervalNum < intervalCount) ?
        nextInterval_opt(GAMMA, x, arr, currInterval, Ilen, I_prev, IOffset, advanceInterval) : MAX_LABEL;

    residualVal = (residualNum < residualCount) ?
        nextResidual_opt(ZETA_3, x, arr, currResidual, r_prev, residualOffset, advanceResidual) : MAX_LABEL;

    // 选择最小值，更新 flag 和 residualNum
    int result = min3_with_flag(multVal, intervalVal, residualVal, flag);
    if (flag == 3) residualNum++;
    return result;
}
...
// 迭代版本核心函数
__device__ int optimized_nextRefValue(
    int* GAMMA,
    int* ZETA_3,
    int& x,
    uint8_t* arr,
    ...
    RefState stack[4];
    int stack_top = 0;
    int final_result = MAX_LABEL;

    // 初始化根状态
    stack[0] = {static_cast<int8_t>(start_ind), PHASE_INITIAL, 0, 0, 0};
    stack_top = 1;

    while (stack_top > 0) {
        RefState& current = stack[stack_top - 1];
        NodeState* curr_node = &refChain[current.ind];

        switch (current.phase) {
        case PHASE_INITIAL: {
            // 终止条件检查
            if (curr_node->blockNum > curr_node->blockCount) {
                final_result = MAX_LABEL;
                stack_top--;
                break;
            }

            // 判断是否需要处理引用链
            if (refChain[current.ind + 1].ref == 0) {
                // 非引用路径处理
                if (curr_node->currblock == 0 && curr_node->ref_flag == 0) {
                    curr_node->blockNum++;
                    if (curr_node->blockNum == curr_node->blockCount - 1) {
                        curr_node->blockNum++;
                    }

                    // 读取新块数据
                    curr_node->currblock = readGamma(GAMMA, arr, &curr_node->refOffset) + 1;
                    
                    // 处理子块（展开小循环）
                    for (int i = 0; i < curr_node->currblock && i < 4; ++i) {
                        nextValue(GAMMA, ZETA_3, refChain[current.ind+1].x, arr, 
                                &refChain[current.ind+1]);
                    }

                    curr_node->blockNum++;
                    if (curr_node->blockNum == curr_node->blockCount - 1) {
                        curr_node->currblock = refChain[current.ind+1].d - curr_node->cpt;
                    } else {
                        curr_node->currblock = readGamma(GAMMA, arr, &curr_node->refOffset) + 1;
                    }
                }
                ...
                // 进入结果获取阶段
                current.phase = PHASE_AFTER_REF_CALL;
                if (current.ind < 3) { // 确保不超过最大深度
                    stack[stack_top++] = {static_cast<int8_t>(current.ind+1), 
                                         PHASE_INITIAL, 0, 0, 0};
                }
            } else {
                // 引用路径处理
                if (curr_node->currblock == 0) {
                    curr_node->blockNum++;
                    if (curr_node->blockNum == curr_node->blockCount - 1) {
                        curr_node->blockNum++;
                    }
        ...
        case PHASE_AFTER_REF_CALL: {
            // 获取子层结果
            if (stack_top > 1) {
                current.temp = stack[stack_top-1].temp;
                stack_top--; // 弹出子状态
            }
            ...
            // 确定返回结果
            if (current.ind == start_ind) {
                final_result = current.temp;
                stack_top--;
            } else {
                // 将结果传递到上层
                stack[stack_top-2].temp = current.temp;
                current.phase = PHASE_FINALIZE;
            }
            break;
        }

        case PHASE_AFTER_INTERVAL: {
            // 处理ref_nextValue中的区间比较逻辑
            if (current.ref_flag == 0) {
                int ref_val = current.temp;
                current.temp = nextValue(GAMMA, ZETA_3, x, arr, curr_node);
                current.ref_flag = (ref_val < current.temp) ? 1 : 2;
                final_result = (ref_val < current.temp) ? ref_val : current.temp;
            } else if (current.ref_flag == 1) {
                int interval_val = (curr_node->flag == 1) ? curr_node->currMult :
                                   (curr_node->flag == 2) ? curr_node->currInterval :
                                   curr_node->currResidual;
                current.temp = nextValue(GAMMA, ZETA_3, x, arr, curr_node);
                current.ref_flag = (current.temp < interval_val) ? 1 : 2;
                final_result = (current.temp < interval_val) ? current.temp : interval_val;
            } else {
                current.temp = nextValue(GAMMA, ZETA_3, x, arr, curr_node);
                final_result = current.temp;
            }
            ...
            // 回传结果
            if (current.ind != start_ind) {
                stack[stack_top-2].temp = final_result;
            }
            stack_top--;
            break;
        }
...
__device__ void get_dest(long* NodeOffset, int off ,int dest_size, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
...
__device__ void travel(long* NodeOffset , int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
  ...
  //  uint64_t tmp_t = getOffset(x1, 6, d_t_lowerBitsVector, d_t_selectUpper);
    ...
    if (ref > 0) {
        //  printf("here travel before\n"); 
           ...
           get_dest(NodeOffset,off,d, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1);
...
__global__ void travelGraph(int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
    __shared__ long NodeOffset[512]; // 使用共享内存存储当前处理的节点索引
    ...
    ////__syncthreads();
       ...
       if (n > MAX_LABEL)   return; 
        // 处理节点 nodeIdx 的邻居
         travel(NodeOffset,0, dv2, GAMMA, ZETA_3, n, arr,  s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
