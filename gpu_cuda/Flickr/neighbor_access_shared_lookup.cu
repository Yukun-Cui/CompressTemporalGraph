...
__device__ __forceinline__ uint64_t v_func(uint64_t v1, uint64_t v2) {
    int64_t d = static_cast<int64_t>(v2) - static_cast<int64_t>(v1);
    uint64_t abs_d = (d ^ (d >> 63)) - (d >> 63);  // 等价于 abs(d)
    uint64_t sign = (d >> 63) & 1ULL;              // 0 if v2 >= v1, 1 if v1 > v2
    return (abs_d << 1) - sign;
}
__device__ __forceinline__ int64_t inv_v_func(uint64_t x) {
    return (x & 1) ? -(int64_t((x >> 1) + 1)) : (x >> 1);
}
...
__device__ __forceinline__ uint64_t readUnary(uint8_t* arr, uint64_t* s) {
    uint64_t pos = *s;
    uint64_t byteIdx = pos >> 3;
    uint8_t bitOffset = pos & 7;
    uint64_t res = 0;

    while (true) {
        uint8_t byte = arr[byteIdx];
        for (int i = bitOffset; i < 8; ++i, ++pos) {
            if ((byte >> (7 - i)) & 1) {
                *s = pos + 1;
                return res;
            }
...
*/
__device__ uint64_t readGamma(int* GAMMA, uint8_t* arr, uint64_t* s,int* gamma_shared) {
    uint64_t byteIndex = *s >> 3;
    uint8_t bitOffset = *s & 7;

    uint16_t result = 0;
    uint8_t b0 = arr[byteIndex];
    uint8_t b1 = arr[byteIndex + 1];
    uint8_t b2 = arr[byteIndex + 2];

    uint16_t raw = ((b0 & ((1 << (8 - bitOffset)) - 1)) << 8) | b1;
    if (bitOffset != 0) {
        raw = (raw << bitOffset) | (b2 >> (8 - bitOffset));
    }
    result = raw;

    int preComp;
     if (result < 6144 && (preComp = gamma_shared[result ]) != 0) {  // 如果数据在共享内存
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
      if ((preComp = GAMMA[result & 0xFFFF]) != 0) {
    ...
    // fallback: standard decoding
    uint64_t cpt = *s;
    uint64_t l = readUnary(arr, &cpt);
    if (l == 0) {
        *s = cpt;
        return 0;
    }

    uint64_t ind = cpt + l - 1;
    uint64_t val = 1ULL << l;

    #pragma unroll 4
    for (uint64_t p = 0; p < l; p++) {
        uint64_t bit_pos = ind - p;
        uint8_t byte = arr[bit_pos >> 3];
        val |= ((byte >> (7 - (bit_pos & 7))) & 1) << p;
    }
...
__device__ uint64_t readZeta_k(int* ZETA_3, uint8_t* arr, uint64_t* s,int * zeta_shared) {
    // 位操作准备
    uint64_t byteIndex = *s / 8;
    uint8_t bitOffset = *s % 8;

    // 向量化三字节提取
    uint32_t data = ((uint32_t)arr[byteIndex] << 16) |
                    ((uint32_t)arr[byteIndex + 1] << 8) |
                    ((uint32_t)arr[byteIndex + 2]);
    data <<= bitOffset;
    uint16_t result = data >> 8;  // 提取高 16 位

    // 查表预解码
    int preComp;
    if (result < 6144 && (preComp = zeta_shared[result & 0xFFFF]) != 0) {  // 如果数据在共享内存
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
      preComp = ZETA_3[result & 0xFFFF];
    if (preComp != 0) {
        *s += preComp >> 16;
        return preComp & 0xFFFF;
    }
    ...
    // 手动解码部分
    const uint8_t k = 3;
    uint64_t h = readUnary(arr, s);  // 读取前缀 h（单比特编码）
    uint64_t pos = *s;
    uint64_t val = 0;
    uint64_t left = 1ULL << (h * k);
    uint64_t len = h * k + k;

    // 手动位提取循环（从高位到低位）
    #pragma unroll 4
    for (uint64_t i = 0; i < len - 1; ++i) {
        uint64_t bit_pos = pos + i;
        uint8_t byte = arr[bit_pos / 8];
    ...
    if (val < left) {
        *s = pos + len - 1;
        ...
        uint64_t lastBit = (arr[(pos + len - 1) / 8] >> (7 - ((pos + len - 1) % 8))) & 1;
        *s = pos + len;
        return (val << 1) + lastBit - 1;
    }
}
/*
...
/*
__device__ uint64_t readZeta_k2(uint8_t* arr, uint64_t* s) {
    const uint64_t pos = *s;
    const uint64_t h = readUnary(arr, s);  // 如果你已优化readUnary，也可以换成 readUnary_opt
    const uint64_t k = 2;
    const uint64_t len = h * k + k;
    const uint64_t left = 1ULL << (h * k);
    ...
    for (int i = 0; i < 32; ++i) {
        if (i >= len - 1) break;
        uint64_t bit_pos = pos + len - 2 - i;
        uint8_t byte = arr[bit_pos / 8];
        val = (val << 1) | ((byte >> (7 - (bit_pos % 8))) & 1);
    }
    ...
    if (val < left) {
        *s = pos + len - 1;
        ...
        uint64_t extra_pos = pos + len - 1;
        uint8_t byte = arr[extra_pos / 8];
        uint8_t bit = (byte >> (7 - (extra_pos % 8))) & 1;
        *s = pos + len;
        res = (val << 1) | bit;
    }
...
int __device__ nextMult(int* GAMMA, int* gamma_shared,int& x, uint8_t* arr, int& currMult, int& len, int& prev, uint64_t* multOffset) {
    if (*multOffset == 0) {
    ...
    if (currMult == -1) {
        currMult = prev = (int)(inv_v_func(readGamma(GAMMA, arr, multOffset,gamma_shared)) + x);
        //  printf("prev:%d\n", prev);
        len = readGamma(GAMMA, arr, multOffset,gamma_shared) + DEFAULT_MIN_MULTIPLE_LENGTH;
        len--;
        return currMult;
    }
    if (len == 0 && currMult != -1) {
        currMult = prev = readGamma(GAMMA, arr, multOffset,gamma_shared) + prev + 1;
        //printf("currMult:%d\n", currMult);
        len = readGamma(GAMMA, arr, multOffset,gamma_shared) + DEFAULT_MIN_MULTIPLE_LENGTH;
        len--;
        return currMult;
...
int __device__ nextInterval(int* GAMMA,int* gamma_shared, int& x, uint8_t* arr, int& currInterval, int& len, int& prev, uint64_t* IOffset) {
    if (*IOffset == 0) {
    ...
    if (currInterval == 0) {
        currInterval = prev = (int)(inv_v_func(readGamma(GAMMA, arr, IOffset,gamma_shared)) + x);
        // printf("iprev:%d\n", prev);
        len = readGamma(GAMMA, arr, IOffset,gamma_shared) + DEFAULT_MIN_INTERVAL_LENGTH;
        len--;
        return currInterval;
    }
    if (len == 0 && currInterval != 0) {
        currInterval = prev = readGamma(GAMMA, arr, IOffset,gamma_shared) + prev + 2;
        len = readGamma(GAMMA, arr, IOffset,gamma_shared) + DEFAULT_MIN_INTERVAL_LENGTH;
        len--;
        return currInterval;
    }
...
//获取连续区间中下一个值    currInterval 当前连续边序号  len 当前连续区间剩余长度  prev 前一个连续边序号
int __device__ nextResidual(int* ZETA_3,int* zeta_shared, int& x, uint8_t* arr, int& currResidual, int& prev, uint64_t* residualOffset) {
    if (*residualOffset == 0) {
        currResidual = MAX_LABEL;
        return  currResidual;
    }
    if (currResidual == 0) {
        currResidual = prev = (int)(inv_v_func(readZeta_k(ZETA_3, arr, residualOffset,zeta_shared)) + x);
        // printf("rprev:%d\n", *residualOffset);
        return currResidual;
    }

    else {
        currResidual = prev = readZeta_k(ZETA_3, arr, residualOffset,zeta_shared) + prev + 1;
        return currResidual;
    }
...
int __device__ nextValue(int* GAMMA, int* ZETA_3,int* gamma_shared,int* zeta_shared, int& x, uint8_t* arr,
    ...
    //第一次调用nextValue：
    if (flag == 0) {
        multValue = nextMult(GAMMA,  gamma_shared,  x, arr, currMult, mlen, m_prev, multOffset);
        //   if(x==112202)
       //    printf("multValue:%d\n", multValue);

        intervalValue = nextInterval(GAMMA, gamma_shared,  x, arr, currInterval, Ilen, I_prev, IOffset);

        residualValue = nextResidual(ZETA_3,  zeta_shared, x, arr, currResidual, r_prev, residualOffset);
    ...
    if (flag == 1) {//上一个是重边中的值 
        if (mlen != 0) {
            ...
            multiplesNum++;
            multValue = multiplesNum < multiplesCount ? nextMult(GAMMA, gamma_shared ,x, arr, currMult, mlen, m_prev, multOffset) : MAX_LABEL;
    ...
    if (flag == 2) {//上一个是连续区间中的值 
        if (Ilen != 0) {
            ...
            intervalNum++;
            multValue = currMult;
            intervalValue = intervalNum < intervalCount ? nextInterval(GAMMA,gamma_shared,  x, arr, currInterval, Ilen, I_prev, IOffset) : MAX_LABEL;
    ...
    if (flag == 3) {//上一个是剩余区间中的值 
        ...
        multValue = currMult;
        intervalValue = currInterval;
        residualValue = residualNum < residualCount ? nextResidual(ZETA_3,  zeta_shared, x, arr, currResidual, r_prev, residualOffset) : MAX_LABEL;
...
int __device__ nextValue(int* GAMMA, int* ZETA_3,int*gamma_shared,int* zeta_shared, int& x, uint8_t* arr,
    ...
    //第一次调用nextValue：
    if (current->flag == 0) {
        multValue = nextMult(GAMMA,gamma_shared,  x, arr, current->currMult, current->mlen, current->m_prev,& current->multOffset);
        //   if(x==112202)
       //    printf("multValue:%d\n", multValue);

        intervalValue = nextInterval(GAMMA,gamma_shared, x, arr, current->currInterval, current->Ilen, current->I_prev,& current->IOffset);

        residualValue = nextResidual(ZETA_3,  zeta_shared, x, arr, current->currResidual, current->r_prev,& current->residualOffset);
    ...
    if (current->flag == 1) {//上一个是重边中的值 
        if (current->mlen != 0) {
            ...
            multValue = current->multiplesNum < current->multiplesCount ? nextMult(GAMMA,gamma_shared,x, arr, current->currMult, current->mlen, current->m_prev, &current->multOffset) : MAX_LABEL;
    ...
    if (current->flag == 2) {//上一个是连续区间中的值 
        if (current->Ilen != 0) {
            ...
            intervalValue = current->intervalNum < current->intervalCount ? nextInterval(GAMMA,gamma_shared,x, arr, current->currInterval, current->Ilen, current->I_prev,& current->IOffset) : MAX_LABEL;
    ...
    if (current->flag == 3) {//上一个是剩余区间中的值 
        ...
        multValue = current->currMult;
        intervalValue = current->currInterval;
        residualValue = current->residualNum < current->residualCount ? nextResidual(ZETA_3,zeta_shared, x, arr, current->currResidual, current->r_prev,& current->residualOffset) : MAX_LABEL;
...
//获取节点x在graph中的开始位置
uint64_t __device__  get_position(int* GAMMA, int* gamma_shared,uint8_t* arr, uint8_t* d_Offsets, int x) {
    uint64_t position = 0;
    uint64_t s = 0;
    for (int i = 0; i < x + 1; i++) {
        position += readGamma(GAMMA, d_Offsets, &s,gamma_shared);
    }
    return position;
}
...
//获取节点x的出度
uint64_t __device__  outdegreeInternal(int* GAMMA,int*gamma_shared, uint8_t* arr, uint8_t* d_Offsets, int x) {
    uint64_t position = 0;
    uint64_t s = 0;
    for (int i = 0; i < x + 1; i++) {
        position += readGamma(GAMMA, d_Offsets, &s,gamma_shared);
    }
    return readGamma(GAMMA, arr, &position,gamma_shared);
}


int __device__ ref_nextValue(int* GAMMA,  int* ZETA_3, int* gamma_shared, int* zeta_shared,int& x, uint8_t* arr, NodeState* refChain, int ind);

// 模拟递归消除的 nextRefValue 函数
int __device__ nextRefValue(int* GAMMA,   int* ZETA_3, int* gamma_shared, int* zeta_shared,int& x, uint8_t* arr, NodeState* refChain, int ind) {
    // 模拟栈
    int stack[4];  // 栈大小为4
    int top = -1;          // 栈顶指针
    int result = 0;
    ...
    while (top >= 0) {
        ...
        if (refChain[ind + 1].ref == 0) {
            if (refChain[ind].currblock == 0 && refChain[ind].ref_flag == 0) {
                ...
                refChain[ind].currblock = readGamma(GAMMA, arr, &refChain[ind].refOffset,gamma_shared) + 1;

                for (int i = 0; i < refChain[ind].currblock; i++) {
                    nextValue(GAMMA, ZETA_3, gamma_shared, zeta_shared, refChain[ind + 1].x, arr, &refChain[ind + 1]);
                }
                refChain[ind].blockNum++;
                if (refChain[ind].blockNum == refChain[ind].blockCount - 1) {
            ...
            result = nextValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, refChain[ind + 1].x, arr, &refChain[ind + 1]);
            ...
            if (refChain[ind].currblock == 0) {
                ...
                for (int i = 0; i < refChain[ind].currblock; i++) {
                    ref_nextValue(GAMMA, ZETA_3, gamma_shared, zeta_shared,refChain[ind + 1].x, arr, refChain, ind + 1);
                }
                refChain[ind].blockNum++;
                if (refChain[ind].blockNum == refChain[ind].blockCount - 1) {
                    refChain[ind].currblock = refChain[ind + 1].d - refChain[ind].cpt;
            ...
            result = ref_nextValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, refChain[ind + 1].x, arr, refChain, ind + 1);
            refChain[ind].currblock--;
            ...
            if (refChain[ind].currblock == 0) {
                ...
                if (refChain[ind].blockNum == refChain[ind].blockCount) {
                    ...
                    refChain[ind].currblock = readGamma(GAMMA, arr, &refChain[ind].refOffset,gamma_shared) + 1;  //printf("else,  currblock:%d\n", refChain[ind].currblock);
                }
                //  cpt += currblock;
...
int __device__ ref_nextValue(int* GAMMA, int* ZETA_3,int* gamma_shared, int* zeta_shared, int& x, uint8_t* arr, NodeState* refChain, int ind) {
    ...
    while (top >= 0) {
        ...
        if (refChain[ind].ref_flag == 0) {
            value_ref = nextRefValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, x, arr, refChain, ind);
            refChain[ind].pre_refvalue = value_ref;
            value_interval = nextValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, x, arr, &refChain[ind]);
        ...
        if (refChain[ind].ref_flag == 1) {
            ...
            refChain[ind].pre_refvalue = value_ref = nextRefValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, x, arr, refChain, ind);
...
enum Phase {
    PHASE_INITIAL,
    PHASE_AFTER_REF_CALL,
    PHASE_AFTER_INTERVAL,
    PHASE_FINALIZE
};

// 显式栈状态结构体（紧凑设计）
struct __align__(8) RefState {
    int8_t ind;         // 当前参考链层级 0-2
    Phase phase;        // 当前阶段
    int8_t ref_flag;    // 记录ref_nextValue状态
    int16_t currblock;  // 当前块计数
    int32_t temp;       // 临时值存储
};
 

// 获取参考节点的邻居序列，存于dest,用于解读参考块 

__device__ void get_dest(  int off ,int dest_size, bool* dv2, int* GAMMA, int* ZETA_3,int* gamma_shared,int* zeta_shared, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc)
{
  ...
  // printf("get dest_size: %d\n", dest_size);
    ...
    if (!isfc) {
        ...
        //   printf("here isfc \n" );
        *s = getOffset(x, gef_low_len, d_lowerBitsVector, d_selectUpper);
        //    printf("s:%lu\n", *s);
    }
    // printf("here travel\n");
    int d,//出度
   ...
   // printf("here before read d \n");
    int blockCount = 0, cptblocks = 0, block_size,
        //  i,
        cpt = 0, cptdecoded = 0;
    d = readGamma(GAMMA, arr, s,gamma_shared);
    extraCount = d;
    ...
    //if (refChain == NULL)  printf("malloc error\n");
    for (int i = 0; i < 4; i++) {
        refChain[i].x = 0;
        refChain[i].d = 0;
        refChain[i].ref = 0;
        refChain[i].cpt = 0;
        refChain[i].currblock = 0;
        refChain[i].blockCount = 0;
        //refChain[i].valid = false;
        refChain[i].refOffset = 0;
        refChain[i].blockNum = 0;
        refChain[i].ref_flag = 0;
        refChain[i].pre_refvalue = 0;
        refChain[i].currMult = 0;
        refChain[i].mlen = 0;
        refChain[i].m_prev = 0;
        refChain[i].multOffset = 0;
        refChain[i].multiplesCount = 0;
        refChain[i].multiplesNum = 0;
        refChain[i].currInterval = 0;
        refChain[i].Ilen = 0;
        refChain[i].I_prev = 0;
        refChain[i].IOffset = 0;
        refChain[i].intervalCount = 0;
        refChain[i].intervalNum = 0;
        refChain[i].currResidual = 0;
        refChain[i].r_prev = 0;
        refChain[i].residualOffset = 0;
        refChain[i].residualCount = 0;
        refChain[i].residualNum = 0;
        refChain[i].flag = 0;
    }
    for (int i = 0; i < 4; i++) refChain[i].currMult = -1;
    // printf("here travel\n");
   // int RefChain[3]; //记录参考链中的ref值，3为最大参考链长度
    ...
    while (tmp_ref > 0) {//参考链未到底
       ...
       //   uint64_t tmp_offset = 0;
        ...
        uint8_t cv = 1;
        refChain[ChainLen].blockCount = blockCount = readGamma(GAMMA, arr, &tmp_rpos,gamma_shared);
        //   printf("参考块数量：%d", blockCount);
        refChain[ChainLen].refOffset = tmp_rpos;
        cptblocks = 0; cpt = 0; cptdecoded = 0;
        //  refOffset =  tmp_rpos;
        if (blockCount != 0) {
            int currblock = readGamma(GAMMA, arr, &refChain[ChainLen].refOffset,gamma_shared);
            refChain[ChainLen].currblock = currblock;
            // printf("blockCount:%d\n", blockCount);

            while (cptblocks < blockCount) {
                block_size = readGamma(GAMMA, arr, &tmp_rpos,gamma_shared) + (cptblocks == 0 ? 0 : 1);
        ...
        if (cv) {
            //     printf("x-ref:%d\n", x - ref);
          //  uint64_t tmp_offsets = off-ref>=0 ? NodeOffset[off-ref]: getOffset(x - ref, 7, d_lowerBitsVector, d_selectUpper);
            uint64_t tmp_offsets = getOffset(originV - tmp_ref, gef_low_len, d_lowerBitsVector, d_selectUpper);
            // tmp_offset = tmp_offsets;
            cptdecoded += readGamma(GAMMA, arr, &tmp_offsets,gamma_shared) - cpt; //dlen: 这里表示被参考节点的出度 可以直接读取x-ref的开始位置，然后readgamma
        }
        ...
        if (extraCount > 0) {
            ...
            refChain[ChainLen].multiplesCount = multiplesCount = readGamma(GAMMA, arr, &tmp_rpos,gamma_shared);
            ...
            if (multiplesCount != 0) {
                ...
                for (int i = 0; i < multiplesCount; i++) {
                    ...
                    extraCount -= readGamma(GAMMA, arr, &tmp_rpos,gamma_shared) + DEFAULT_MIN_MULTIPLE_LENGTH;
        ...
        int intervalCount = 0;
        if (extraCount > 0) {
            refChain[ChainLen].intervalCount = intervalCount = readGamma(GAMMA, arr, &tmp_rpos,gamma_shared);
            ...
            if (intervalCount != 0) {
                ...
                for (int i = 0; i < intervalCount; i++) {
                    skipGamma(arr, &tmp_rpos);
                    extraCount -= readGamma(GAMMA, arr, &tmp_rpos,gamma_shared) + DEFAULT_MIN_INTERVAL_LENGTH;
                }
            }
        }
        if (extraCount > 0)
        ...
        //tmp_rpos = off-x+originV>=0?NodeOffset[off - x + originV]:getOffset(originV, 7, d_lowerBitsVector, d_selectUpper);
         tmp_rpos =  getOffset(originV, gef_low_len, d_lowerBitsVector, d_selectUpper);
        // printf("originV:%d\n", originV);
         //refVpos[ChainLen] = tmp_rpos;
        extraCount = readGamma(GAMMA, arr, &tmp_rpos,gamma_shared);
...
result = ref_nextValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, x, arr, refChain, ind);
         if (result < 0 || result >  MAX_LABEL) {
            printf("result:%d, %d\n", result, x);  return;
        } 
        ...
        //  if(x==4)
            if(x==4)  printf(" result:%d    timestamp:%d\n", result, timestamp);
         //printf("%d,\n", result ); 
...
__device__ void travel( int off, bool* dv2, int* GAMMA, int* ZETA_3,int* gamma_shared,int* zeta_shared, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper)
{
     if (x >= MAX_LABEL) return;
    //curand_init(2, n, 0, &state);
    //if(x%1000==0) printf("%d\n",x);
    // 生成一个在范围 [0, nodenum) 内的随机数
    //int randomValue = curand(&state) % MAX_LABEL;
    // if (n > 33) return;
      //节点6584：
      //*s = 66364008;
      //*t_pos = 83395934;
   ...
   //  if (n == 31) printf("position:%lu", *s);
    // *t_pos = T_offset[x1];
    uint64_t ref_s;
  //  uint64_t tmp_s = off>=0?NodeOffset[off] : getOffset(x1, 7, d_lowerBitsVector, d_selectUpper); 
    
	uint64_t tmp_s = getOffset(x1, gef_low_len, d_lowerBitsVector, d_selectUpper);
	 
    uint64_t tmp_t = getOffset(x1, tef_low_len, d_t_lowerBitsVector, d_t_selectUpper);
       

  //  uint64_t tmp_t = getOffset(x1, 6, d_t_lowerBitsVector, d_t_selectUpper);
    // printf("错了错了\n");


    ref_s = tmp_s;
    int d, ref;
    d = readGamma(GAMMA, arr, &tmp_s,gamma_shared);
    if (d < 4) dv2[x1] = true;
    // printf("d:%d\n", d);
    if (d == 0) {
        // dv2[x1] = true;
   ...
   return;
           get_dest( off,d, dv2, GAMMA, ZETA_3,gamma_shared,zeta_shared, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1);
  ...
  // printf("here n: %d\n",n);
        ...
        if (extraCount > 0) {
            ...
            multiplesCount = readGamma(GAMMA, arr, &tmp_s,gamma_shared);
            //   cout << "multiplesCount" << multiplesCount << endl;
            ...
            if (multiplesCount != 0) {
                ...
                for (int i = 0; i < multiplesCount; i++) {
                    skipGamma(arr, &tmp_s);
                    extraCount -= readGamma(GAMMA, arr, &tmp_s,gamma_shared) + DEFAULT_MIN_MULTIPLE_LENGTH;
        ...
        int intervalCount = 0;
        if (extraCount > 0) {
            intervalCount = readGamma(GAMMA, arr, &tmp_s,gamma_shared);
            ...
            if (intervalCount != 0) {
                ...
                for (int i = 0; i < intervalCount; i++) {
                    skipGamma(arr, &tmp_s);
                    extraCount -= readGamma(GAMMA, arr, &tmp_s,gamma_shared) + DEFAULT_MIN_INTERVAL_LENGTH;
                }
            }
        }
        if (extraCount > 0)
        ...
        // int cumt = 0;
        int currMult = -1, mlen = 0, m_prev=0, multiplesNum = 0;
        int currInterval = 0, Ilen = 0, I_prev=0, intervalNum = 0;
        int currResidual = 0, r_prev=0, residualNum = 0, flag = 0;
        int index = 0;
        /* NodeState* Node = (NodeState*)malloc(sizeof(NodeState) * 1);
        Node->currMult = currMult; Node->mlen = mlen; Node->m_prev = m_prev; Node->multOffset = multOffset;
        Node->multiplesCount = multiplesCount; Node->multiplesNum = multiplesNum; Node->currInterval = currInterval; Node->Ilen = Ilen;
        Node->I_prev = I_prev; Node->IOffset = IOffset; Node->intervalCount = intervalCount; Node->intervalNum = intervalNum;
        Node->currResidual = currResidual; Node->r_prev = r_prev; Node->residualOffset = residualOffset; Node->residualCount = residualCount;
        Node->residualNum = residualNum; Node->flag = flag;    */
        while (index < d) {
           //  result = nextValue(GAMMA, ZETA_3, x1, arr, Node);
              result = nextValue(GAMMA, ZETA_3,gamma_shared, zeta_shared, x1, arr,
            ...
            if (cumt < 3804) {
                ...
                printf("result:%d\n", result);
                if (result != dv2[cumt]) printf("error:%d %d\n", result,dv2[cumt]);
                cumt++;
            }
            if (cumt == 3804) break;
            */
            // if(n==9999) printf("result:%d\n", result);
           //cout<<"r_count1"<<r_count1<<endl;
          //  if(v2_[count_r]!=result) cout<<"here"<<" ";
          // cout<<"here"<<endl;
         //   if (*t_pos > 13701410 * 8) {
           //     printf("out of range\n" );
            //}
...
__global__ void travelGraph(int off, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
     
__shared__ int gamma_shared[6144];  // 存储 GAMMA 数组前 6144 个元素
    __shared__ int zeta_shared[6144];   // 存储 ZETA 数组前 6144 个元素
    //int tid = threadIdx.x;
    //int blockStart = 512 * nodesPerBlockMultiplier * blockIdx.x;
    //int blockEnd = min(blockStart + 512 * nodesPerBlockMultiplier, MAX_LABEL);
    //int u = blockDim.x * blockIdx.x + threadIdx.x;
    //if (u > MAX_LABEL) return;
    //NodeOffset[tid] = getOffset(u, 7, d_lowerBitsVector, d_selectUpper);
    //// 初始化共享内存中的当前处理节点索引
    ////__syncthreads();
       int n = off + blockDim.x * blockIdx.x + threadIdx.x;
       if (n > MAX_LABEL) return;
       if (threadIdx.x == 0) {
        // 加载 GAMMA 和 ZETA 数组前 6144 个元素到共享内存
        for (int i = 0; i < 6144; i++) {
            gamma_shared[i] = GAMMA[i];  // 加载 GAMMA 数组前 6144 个元素
            zeta_shared[i] = ZETA_3[i];  // 加载 ZETA 数组前 6144 个元素
        }
    }
    //else if(threadIdx.x < 6144) {
         
    //    zeta_shared[threadIdx.x] = ZETA_3[threadIdx.x];   // 加载 ZETA 数组前 6144 个元素
   // }
    __syncthreads();  // 确保所有线程加载数据后才进行下一步
        // 处理节点 nodeIdx 的邻居
         travel( 0, dv2, GAMMA, ZETA_3,gamma_shared,zeta_shared, n, arr,  s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    //__shared__ int currentNodeIdx; // 使用共享内存存储当前处理的节点索引
    ...
    //// 初始化共享内存中的当前处理节点索引
    //if (tid == 0) {
    //    currentNodeIdx = blockStart + 512;
    //}
    //__syncthreads();
    // 
    //    travel(NodeOffset, off, dv2, GAMMA, ZETA_3, blockStart + tid, arr, s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    //
    //while (true) {
    //    // 通过原子操作来安全地更新共享内存中的当前处理节点索引
    //    int nodeIdx = atomicAdd(&currentNodeIdx, 1);

    //    if (nodeIdx >= blockEnd) break;

    //    // 处理节点 nodeIdx 的邻居
    //    
    //            travel(NodeOffset, off, dv2, GAMMA, ZETA_3, nodeIdx  , arr, s, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper)  ;
    //        
    //     
    //} 
...
int main() {
    ...
    // int* hostArray = new int[234 * 10000]; // 主机上的数组
     //加载gamma.in.16文件
    FILE* gamma_in_file = fopen(gamma_file_path, "r");
    int gamma_value;
    int gif_i = 0;
   ...
   //加载zeta.in.16文件
    FILE* zeta_in_file = fopen(zeta_file_path, "r");
    int zeta_value;
    int zif_i = 0;
    ...
    FILE* file = fopen(graph_file_path, "rb"); //10,019,548  54,523,326
    ...
    fclose(file);
    //加载时间戳文件
    FILE* file2 = fopen(timestamps_file_path, "rb"); //13,701,410  22,549,005
    if (file2 == NULL) {
    ...
    FILE* low_file = fopen(g_low_file_path, "rb"); // 2,262,374 
    if (low_file == NULL) {
    ...
    fclose(low_file);
    FILE* upper_file = fopen(g_upper_file_path, "rb"); //749,160
    if (upper_file == NULL) {
    ...
    //时间戳的偏移列表
    FILE* t_low_file = fopen(t_low_file_path, "rb"); // 2,262,374 1,939,177 
    if (t_low_file == NULL) {
        printf("Failed to open the low file.\n");
    ...
    fclose(t_low_file);
    FILE* t_upper_file = fopen(t_upper_file_path, "rb"); //749,160 675,525
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    int block_size = 256;
    int grid_size = MAX_LABEL / block_size + 1;
    int off = 0;
    CHECK(cudaEventRecord(c_start));
    cudaEventQuery(c_start);




    travelGraph << < grid_size, 256, 0 >> > (off, dv2, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    ...
    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());
    CHECK(cudaEventRecord(c_stop));
    CHECK(cudaEventSynchronize(c_stop));
    float elapsed_time;
    CHECK(cudaEventElapsedTime(&elapsed_time, c_start, c_stop));
    printf("Time = %g ms.\n", elapsed_time);
    CHECK(cudaEventDestroy(c_start));
    CHECK(cudaEventDestroy(c_stop));
    bool* h_dv2;
    h_dv2 = (bool*)malloc(MAX_LABEL * sizeof(bool));
    CHECK(cudaMemcpy(h_dv2, dv2, MAX_LABEL * sizeof(bool), cudaMemcpyDeviceToHost));


    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    auto duration1 = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start1).count();
    auto duration2 = std::chrono::duration_cast<std::chrono::nanoseconds>(start2 - start1).count();
    std::cout << "Time taken: " << std::dec << duration << " nanoseconds" << std::endl;
    std::cout << "Time taken: " << std::dec << duration1 << " nanoseconds" << std::endl;
    std::cout << "Time taken: " << std::dec << duration2 << " nanoseconds" << std::endl;
    return;

    cudaError_t cudaStatus = cudaGetLastError();
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "CUDA kernel function running error: %s\n", cudaGetErrorString(cudaStatus));
    }
