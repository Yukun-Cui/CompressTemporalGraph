...
__global__ void travelGraph(int off, bool* dv2,int*  d_task_index, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
      ...
      int n = off + blockDim.x * blockIdx.x + threadIdx.x;
       if (n > MAX_LABEL ) return;
        ...
        // 处理节点 nodeIdx 的邻居
          ...
          while (true) {
            int u = atomicAdd(d_task_index, 1);  // 全局任务分配
              if (u > MAX_LABEL ) break;

                travel (  0, dv2, GAMMA, ZETA_3, u, arr, s, d_Timestamps, t_pos,
...
__global__ void travelGraph2(int off, bool* dv2, int*  d_task_index,int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
          ...
          while (true) {
            int u = atomicAdd(d_task_index, 1);  // 全局任务分配
              if (u > MAX_LABEL ) break;
                ...
                travel2(  0, dv2, GAMMA, ZETA_3, u, arr, s, d_Timestamps, t_pos,
               d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
        }
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    cudaMemset(g_pos, 0, sizeof(uint64_t));
            int* d_task_index;
cudaMalloc(&d_task_index, sizeof(int));
cudaMemset(d_task_index, 0, sizeof(int));
int* d_task_index2;
cudaMalloc(&d_task_index2, sizeof(int));
cudaMemset(d_task_index2, 0, sizeof(int));
    ...
    travelGraph << < grid_size , 512, 0 >> > (off, dv2,d_task_index, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);


    travelGraph2 << < grid_size*8, 512, 0 >> > (off, dv2,d_task_index2, GAMMA, ZETA_3, 7664, Graph, g_pos, d_Timestamps, t_pos, d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
