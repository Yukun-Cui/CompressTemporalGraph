...
__device__ void PR_get_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc, double* Rank, double* newRank)
{
...
__device__ void PR_travel(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper, double* Rank, double* newRank)
{
    // printf("here travel\n" );
    int x1 = x; //此线程编号 
  //  int n =4;
  //if(i==1)
   ...
   // if (x1 == 794106)
    ...
    if (ref > 0) {
        ...
        //BFS_get_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, d_Fa, d_Xa, d_time, p, d_Ca, changed, d_currentQueue, d_nextQueue, d_queueSize, d_nextQueueSize);
        PR_get_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, Rank, newRank);
        return;
    }
    //7664
   //*s = 72151351;
...
__global__ void getNewRank(
    ...
    double* Rank, double* newRank) { //Fa节点是否处于当前层次，Xa是否已被遍历，d_time节点被遍历的时间，p节点的前置节点，Ca距离 
  ...
  //if(i==1) 
    ...
    newRank[u] = (1.0 - dampingFactor) / MAX_LABEL;

    //目前的思路： 不使用下面的循环，在之前的核函数的基础上，获取到result之后，进行循环内的条件判断。
    PR_travel(off, i, offsetGraph, offsetTimestamps,
...
__global__ void computeSum(double* array, double* sum, int size) {
    __shared__ double sharedSum[256];
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    int tid = threadIdx.x;

    sharedSum[tid] = (idx < size) ? array[idx] : 0.0;
    __syncthreads();
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    // PageRank ================================================================================================================================
    double* h_Rank = (double*)malloc(MAX_Vertex * sizeof(double));

    // Initialize host array
    double init_value = 1.0 / MAX_Vertex;
    for (int i = 0; i < MAX_Vertex; ++i) {
        h_Rank[i] = init_value;
    }
    double* Rank, * newRank;

    CHECK(cudaMalloc((void**)&Rank, MAX_Vertex * sizeof(double)));
    CHECK(cudaMalloc((void**)&newRank, MAX_Vertex * sizeof(double)));
    CHECK(cudaMemcpy(Rank, h_Rank, MAX_Vertex * sizeof(double), cudaMemcpyHostToDevice));
   ...
   //  CHECK(cudaMemcpy(d_t_upperBits, t_upperBits, 675525 * sizeof(uint8_t), cudaMemcpyHostToDevice));
    ...
    printf("Starting simple parallel PageRank.\n");



    auto PR_start = std::chrono::steady_clock::now();
    int iternum = PAGERANK_ITERATIONS;
    ...
    while (iter < iternum) {
        ...
        CHECK(cudaMemset(d_sum, 0, sizeof(double)));
            ...
            //    cuLaunchKernel(simpleBfs, 800, 1, 1,  512, 1, 1, 0, 0, args, 0)  ;
             ...
             getNewRank << <grid_size, 512 >> > (pr_off, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
                d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
                Rank, newRank);
            CHECK(cudaGetLastError());
            CHECK(cudaDeviceSynchronize());
        ...
        computeSum << <blocksPerGrid, threadsPerBlock >> > (  Rank, d_sum,MAX_Vertex);
        // Normalize the Rank array on the device
        normalizeArray << <blocksPerGrid, threadsPerBlock >> > (Rank, d_sum, MAX_Vertex);
        CHECK(cudaDeviceSynchronize());
        // cudaEventRecord(stop, streams[i]);
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&milliseconds, start, stop);
    ...
    long PR_duration = std::chrono::duration_cast<std::chrono::milliseconds>(PR_end - PR_start).count();
    printf("PageRank Elapsed time in milliseconds : %li ms.\n", PR_duration);
