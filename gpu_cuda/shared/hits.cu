...
__device__ void HITS_get_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc, double* hub, double* authority)
{
   ...
   // printf("here berfore travel\n");
    ...
    while (index < d) {
        ...
        //  printf("%d,\n", result ); 
        if (result < MIN_GRAPH_VERTEX || result > MAX_Vertex) {
            printf("result:%d, %d\n", result, x); // return;
        }
        if (result != previous_v && result <= MAX_Vertex) {
            hub[x] += authority[result];
...
__device__ void HITS_travel(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper, double* hub, double* authority)
{
    // printf("here travel\n" );
    int x1 = x; //此线程编号 
  //  int n =4;
  //if(i==1)
   ...
   // if (x1 == 794106)
    ...
    if (ref > 0) {
        ...
        HITS_get_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, hub, authority);
        return;
    }
    else {
        int result;
        ...
        while (index < d) {
            ...
            if (result != previous_v && result <= MAX_Vertex && result >= ACTIVE_VERTEX_BEGIN) {
                hub[x] += authority[result];
                //double contribution = dampingFactor * Rank[x] * (1.0 - exp(-0.1 * timestamp)) / d;
               // atomicAdd(&newRank[result], contribution);
...
__device__ void update_authority_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc, double* hub, double* authority)
{
   ...
   // printf("here berfore travel\n");
    ...
    while (index < d) {
        ...
        if (result != previous_v && result <= MAX_Vertex) {
            atomicAdd(&authority[result], hub[x]);
...
__device__ void update_authority(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper, double* hub, double* authority)
{
    // printf("here travel\n" );
    int x1 = x; //此线程编号 
  ...
  //if(i==1)
   ...
   //  if (n == 31) printf("position:%lu", *s);
    ...
    if (ref > 0) {
        //  printf("here travel before\n");
        update_authority_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, hub, authority);
        return;
    }
    else {
        int result;
...
__global__ void get_new_hub(
    ...
    double* hub, double* authority) { //Fa节点是否处于当前层次，Xa是否已被遍历，d_time节点被遍历的时间，p节点的前置节点，Ca距离 

    int thid = blockDim.x * blockIdx.x + threadIdx.x; //此线程编号 
  //  int n =4;
  //if(i==1) 
    //printf("n:%d\n", MAX_LABEL * (i + 1) / 2);
    if (thid >= MAX_LABEL)  return;
    ...
    HITS_travel(off, i, offsetGraph, offsetTimestamps,
        dv2, GAMMA, ZETA_3, u, arr, s, d_Timestamps, t_pos,
        d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
        hub, authority);


}
__global__ void get_new_authority(
    int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps,
    bool* dv2, int* GAMMA, int* ZETA_3, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper,
    double* hub, double* authority) { //Fa节点是否处于当前层次，Xa是否已被遍历，d_time节点被遍历的时间，p节点的前置节点，Ca距离 
  ...
  //if(i==1) 
    ...
    //目前的思路： 不使用下面的循环，在之前的核函数的基础上，获取到result之后，进行循环内的条件判断。
    update_authority(off, i, offsetGraph, offsetTimestamps,
        dv2, GAMMA, ZETA_3, u, arr, s, d_Timestamps, t_pos,
        d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
        hub, authority);
...
__global__ void computeNormKernel(double* scores, double* norm, int num_vertices) {
...
__global__ void finalizeNormKernel(double* norm) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        norm[0] = sqrt(norm[0]);
    }
}
// CUDA kernel to normalize an array
__global__ void normalizeArray(double* array, double* sum, int size) {
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    printf("Starting simple parallel HITS.\n");
    double* h_hub = (double*)malloc(MAX_Vertex * sizeof(double));
    ...
    CHECK(cudaMalloc((void**)&hub, MAX_Vertex * sizeof(double)));
    CHECK(cudaMalloc((void**)&authority, MAX_Vertex * sizeof(double)));
    CHECK(cudaMemcpy(hub, h_hub, MAX_Vertex * sizeof(double), cudaMemcpyHostToDevice));
    CHECK(cudaMemcpy(authority, h_hub, MAX_Vertex * sizeof(double), cudaMemcpyHostToDevice));
    double* d_sum;
    CHECK(cudaMalloc((void**)&d_sum, sizeof(double)));
    auto HITS_start = std::chrono::steady_clock::now();


    int iternum = HITS_ITERATIONS;
    int threadsPerBlock = 256;
    int blocksPerGrid = (MAX_Vertex + threadsPerBlock - 1) / threadsPerBlock;
    int  h_iter = 0;
    int HITS_off = 0;
    //int threadsPerBlock = 256;
   // int blocksPerGrid = (MAX_Vertex + threadsPerBlock - 1) / threadsPerBlock;
    while (h_iter < iternum) {
        h_iter++;
        HITS_off = 0;
        CHECK(cudaMemset(d_sum, 0, sizeof(double)));
       


            //    cuLaunchKernel(simpleBfs, 800, 1, 1,  512, 1, 1, 0, 0, args, 0)  ;

            get_new_hub << <grid_size, 512 >> > (HITS_off, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
                d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
                hub, authority);
            CHECK(cudaGetLastError());
            CHECK(cudaDeviceSynchronize());
        ...
        computeNormKernel << <blocksPerGrid, threadsPerBlock >> > (hub, d_sum, MAX_Vertex);
        cudaDeviceSynchronize();
        finalizeNormKernel << <1, 1 >> > (d_sum);
        cudaDeviceSynchronize();
        normalizeArray << <blocksPerGrid, threadsPerBlock >> > (hub, d_sum, MAX_Vertex);
        cudaDeviceSynchronize();
            ...
            get_new_authority << <grid_size, 512 >> > (HITS_off, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
                d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
                hub, authority);
            CHECK(cudaGetLastError());
            CHECK(cudaDeviceSynchronize());
        ...
        CHECK(cudaMemset(d_sum, 0, sizeof(double)));
        computeNormKernel << <blocksPerGrid, threadsPerBlock >> > (authority, d_sum, MAX_Vertex);
        cudaDeviceSynchronize();
        finalizeNormKernel << <1, 1 >> > (d_sum);
        cudaDeviceSynchronize();
        normalizeArray << <blocksPerGrid, threadsPerBlock >> > (authority, d_sum, MAX_Vertex);
        cudaDeviceSynchronize();
        ...
        /*int sum_off = 0;
        for (int j = 0; j < 7; j++) {
            computeSum << <800, 512 >> > (sum_off,Rank, d_sum, NodeNum);
            sum_off += 409600;
        }*/
        // cudaEventRecord(stop, streams[i]);
        // cudaEventSynchronize(stop);
    ...
    auto HITS_end = std::chrono::steady_clock::now();
    long HITS_duration = std::chrono::duration_cast<std::chrono::milliseconds>(HITS_end - HITS_start).count();
