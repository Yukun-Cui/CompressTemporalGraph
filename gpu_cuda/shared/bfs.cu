...
int __device__ nextRefValue(int* GAMMA, int* ZETA_3, int& x, uint8_t* arr, NodeState* refChain, int ind//记录block[0...i-2]的和，初值与第一个块的值相同，即currblock的初值
    ...
    if (refChain[ind + 1].ref == 0) {
      ...
      // printf("here ind+1  ref==0\n");
        int result = nextValue(GAMMA, ZETA_3, refChain[ind + 1].x, arr,
            &refChain[ind + 1]);
        refChain[ind].currblock--;
        //if(x==6) printf("节点5的result in  nextrefValue:%d  , %d \n", result,ind);
    ...
    else
    {
      ...
      //  printf("here else中 ref_nextValue之前的ind.x:%d.%d\n", ind,refChain[ind + 1].x);
        ...
        refChain[ind].currblock--;
        //  printf("refChain[ind + 1].x:%d\n", refChain[ind + 1].x);
       //     if(x==10772124)  printf(" 10772124 result in  nextrefValue:%d ,  %d\n" , result,refChain[ind].currblock);
        if (refChain[ind].currblock == 0) {
            refChain[ind].blockNum++;
...
int __device__ ref_nextValue(int* GAMMA, int* ZETA_3, int& x, uint8_t* arr, NodeState* refChain, int  ind) {//ind 当前节点在参考链中的位置
    ...
    if (refChain[ind].ref_flag == 0) {
        ...
        //     if(x==10772124)  printf("10772124 result in ref_next nextrefValue:%d\n", value_ref);
         //   if(x==19299664)  printf("19299664 result in ref_next nextrefValue:%d\n", value_ref);
          // if(x==6)  printf("6result in  nextrefValue:%d\n", value_ref);
        refChain[ind].pre_refvalue = value_ref;
        // printf("here after nextRefValue\n");
        value_interval =
            nextValue(GAMMA, ZETA_3, x, arr,
                &refChain[ind]);
        // if(x==19299664) printf("19299664 剩余区间值%d\n",value_interval);
     //  if(x==6) printf("6剩余区间值%d\n",value_interval);
    ...
    if (refChain[ind].ref_flag == 1) {
        ...
        //  if(x==8) printf("8value_ref:%d\n", value_ref);
         //  if(x==8) printf("8ref_flag==1 时value_interval:%d\n", value_interval);
         //  if(x==14) printf("ref_flag==1 时refChain[ind].flag:%d\n", refChain[ind].flag);
        if (value_ref < value_interval) {
            refChain[ind].ref_flag = 1;
            return value_ref;
        }
 ...
 int main() {
  ...
  //  long long totalDuration = 0;
    ...
    //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
     ...
     //设置源节点
     int startVertex = BFS_START_VERTEX;
     int s_Ca = 0;
     bool  s_Fa = true;
     int s_time = -1;
     uint64_t* d_timeAndP;
