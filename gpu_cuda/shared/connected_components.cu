...
int __device__ nextMult(int* GAMMA, int& x, uint8_t* arr, int& currMult, int& len, int& prev, uint64_t* multOffset) {
    ...
    if (currMult == MULTIPLE_INITIAL_VALUE) {
    ...
    if (len == 0 && currMult != MULTIPLE_INITIAL_VALUE) {
...
//获取参考列表中的下一个邻居节点序号
int __device__ nextRefValue(int* GAMMA, int* ZETA_3, int& x, uint8_t* arr, NodeState* refChain, int ind//记录block[0...i-2]的和，初值与第一个块的值相同，即currblock的初值
) {
    //printf("here 进入nextrefvalue\n");
    if (refChain[ind].blockNum > refChain[ind].blockCount) {
        return MAX_LABEL; //参考块已输出完
    }

    if (refChain[ind + 1].ref == 0) {
        //  printf("here 参考节点ref为0\n");
          //首个参考块为0
        if (refChain[ind].currblock == 0 && refChain[ind].ref_flag == 0) {
            ...
            //  printf("refChain[ind].refOffset:%lu\n", refChain[ind].refOffset);
             // printf("currblock:%d\n", refChain[ind].currblock);
              // cpt += currblock;
            for (int i = 0; i < refChain[ind].currblock; i++)
                nextValue(GAMMA, ZETA_3, refChain[ind + 1].x, arr,
                    &refChain[ind + 1]);
            //   printf("here nextvalue结束\n");
               //更新出下个有效块的信息
            refChain[ind].blockNum++;
            if (refChain[ind].blockNum == refChain[ind].blockCount - 1) {
        ...
        //参数信息在调用nextrefvalue前获取   //后续考虑被参考节点也是ref>0 可能需要写nextvalue2函数
      // printf("here ind+1  ref==0\n");
        int result = nextValue(GAMMA, ZETA_3, refChain[ind + 1].x, arr,
            &refChain[ind + 1]);
        refChain[ind].currblock--;
        ...
        if (refChain[ind].currblock == 0) {
            refChain[ind].blockNum++;
            //当前块为倒数第一块，切倒数第一块为有效块，仅剩一个无效的隐藏块，直接跳过
            if (refChain[ind].blockNum == refChain[ind].blockCount) {
                refChain[ind].blockNum++; //此时blockNum == blockCount+1
            ...
            if (refChain[ind].blockNum == refChain[ind].blockCount) {
                ...
                refChain[ind].currblock = refChain[ind + 1].d - refChain[ind].cpt;
                //    printf("refChain[ind + 1]。currblock:%d\n", refChain[ind].currblock);
            }
            else refChain[ind].currblock = readGamma(GAMMA, arr, &refChain[ind].refOffset) + 1;
            //  cpt += currblock;
    ...
    else
    {
        ...
        //参数信息在调用nextrefvalue前获取   //后续考虑被参考节点也是ref>0 可能需要写nextvalue2函数
      // printf("here ind+1  ref==0\n");
      //  printf("here else中 ref_nextValue之前的ind.x:%d.%d\n", ind,refChain[ind + 1].x);
        int result = ref_nextValue(GAMMA, ZETA_3, refChain[ind + 1].x, arr, refChain, ind + 1);
        refChain[ind].currblock--;
        //  printf("refChain[ind + 1].x:%d\n", refChain[ind + 1].x);
         //  printf("result in  nextrefValue:%d\n", result);
        if (refChain[ind].currblock == 0) {
...
//ref>0的节点的nextvalue函数
int __device__ ref_nextValue(int* GAMMA, int* ZETA_3, int& x, uint8_t* arr, NodeState* refChain, int  ind) {//ind 当前节点在参考链中的位置

    //printf("here ref_nextValue just in%d \n",ind );
     // printf("ref_flag in ref_nextvalue:%d\n", refChain[ind].ref_flag);
    int value_ref;
    int value_interval;
    if (refChain[ind].ref_flag == 0) {
        //  printf("here x  Value:%d \n",x );
        //  printf("here refChain[ind].pre_refvalue  Value:%d \n", refChain[ind].pre_refvalue);

        value_ref = nextRefValue(GAMMA, ZETA_3, x, arr, refChain, ind);
        // if (x == 112202) printf("value_ref:%d\n", value_ref);
        refChain[ind].pre_refvalue = value_ref;
        // printf("here after nextRefValue\n");
        value_interval =
            nextValue(GAMMA, ZETA_3, x, arr,
                &refChain[ind]);
    ...
    if (refChain[ind].ref_flag == 1) {
        ...
        else if (refChain[ind].flag == 3) { //printf("here currResi:%d\n", refChain[ind].currResidual); 
            value_interval = refChain[ind].currResidual;
        }
        refChain[ind].pre_refvalue = value_ref =
            nextRefValue(GAMMA, ZETA_3, x, arr, refChain, ind);
        //  printf("value_ref:%d\n", value_ref);
         // printf("value_interval:%d\n", value_interval);
        if (value_ref < value_interval) {
            refChain[ind].ref_flag = 1;
            return value_ref;
        }
...
__global__ void getExistNode(int* GAMMA, int* ZETA_3, uint8_t* arr, uint64_t* s,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool* isExist, int* cc_count)
{
    ...
    // printf("d:%d\n", d);
    if (d > 9)  isExist[x1] = true;
    else atomicAdd(cc_count, 1);
...
__device__ void BFS_get_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    ...
    int* d_currentQueue, int* d_nextQueue, int* d_queueSize, int* d_nextQueueSize, uint64_t* d_timeAndP, int* component, int componentId)
{
  ...
  // printf("get dest_size: %d\n", dest_size);
   ...
   // NodeState* refChain = (NodeState*)malloc(sizeof(NodeState) * 4);
    ...
    memset(refChain, 0, sizeof(NodeState) * 4);
     for (int i = 0; i < 4; i++) refChain[i].currMult = MULTIPLE_INITIAL_VALUE;
    // printf("here travel\n");
   ...
   // printf("here berfore travel\n");
    ...
    while (index < d) {
        ...
        //  printf("%d,\n", result ); 
        if (result < MIN_GRAPH_VERTEX || result > MAX_LABEL) {
            printf("result:%d, %d\n", result, x);  return;
        }
        if (result != previous_v && result <= MAX_Vertex && result >= ACTIVE_VERTEX_BEGIN) {
            TimeAndP newVal;
            newVal.p = x;
    ...
    int* d_currentQueue, int* d_nextQueue, int* d_queueSize, int* d_nextQueueSize, uint64_t* d_timeAndP, int* component, int componentId)
{
  ...
  //  int n =4;
  //if(i==1)
    if (x1 > MAX_LABEL || x1 < ACTIVE_VERTEX_BEGIN) return;
   ...
   //  if (n == 31) printf("position:%lu", *s);
    ...
    if (ref > 0) {
        ...
        BFS_get_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, d_Fa, d_Xa, d_time, p, d_Ca, changed, d_currentQueue, d_nextQueue, d_queueSize, d_nextQueueSize, d_timeAndP, component, componentId);
  ...
  // printf("here n: %d\n",n);
        ...
        int previous_v = -1;
        // int cumt = 0;
        int currMult = MULTIPLE_INITIAL_VALUE, mlen = 0, m_prev, multiplesNum = 0;
        ...
        while (index < d) {
            ...
            if (result != previous_v && result <= MAX_Vertex && result >= ACTIVE_VERTEX_BEGIN) {
                ...
                if (static_cast<int>(d_timeAndP[x] >> 32) <= timestamp && atomicMin(&d_timeAndP[result], maskednewValLL) > maskednewValLL) {
                    ...
                    if (atomicCAS(&d_Xa[result], 0, 1) == 0) { //第一次遇到节点result
                        int pos = atomicAdd(d_nextQueueSize, 1);
                        d_nextQueue[pos] = result;
                        d_Fa[result] = true;
                        component[result] = componentId;
...
__global__ void Bfs(int level,   //LEVEL 有用吗？
    ...
    int* currentQueue, int* nextQueue, int* queueSize, int* nextQueueSize, uint64_t* d_TimeAndP, int* component, int componentId) { //Fa节点是否处于当前层次，Xa是否已被遍历，d_time节点被遍历的时间，p节点的前置节点，Ca距离 
    ...
    //   if (!d_Xa[thid]) {
      ...
      //目前的思路： 不使用下面的循环，在之前的核函数的基础上，获取到result之后，进行循环内的条件判断。
        ...
        BFS_travel(off, i, offsetGraph, offsetTimestamps,
            ...
            d_Fa, d_Xa, d_time, p, d_Ca, changed,
            currentQueue, nextQueue, queueSize, nextQueueSize, d_TimeAndP, component, componentId);
        //   printf("after bfs \n");

    }

    //else if (d_xa[v]) {
    //    d_fa[component[v]] = true; // 需要用到d_fa中，前端 的 目前已有联通分量个数 个位置
    //}


}

void run_BFS(int v, int* component, int componentId,
    int i, uint64_t offsetGraph, uint64_t offsetTimestamps,
    bool* dv2, int* GAMMA, int* ZETA_3, uint8_t* Graph, uint64_t* g_pos, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper,
    bool* d_Fa, bool* d_Fa2, int* d_Xa, int* d_time, int* p, int* d_Ca, int* changed,
    int* queueSize, int* nextQueueSize, int* d_currentQueue, int* d_nextQueue, int* d_queueSize, int* d_nextQueueSize, uint64_t* d_TimeAndP, int* cc_count)
{
    //将v放入queue中，看看是在这放还是进入run_BFS之前
    *changed = 1;
    int level = 0;
    int b_off = 0;
    while (*queueSize > 0  /*&&level<=1*/) {
        ...
        *cc_count += *queueSize;
        /*for (int j = 0; j < 7; j++) {
             ...
             Bfs << <800, 512, 0 >> > (level, b_off, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
                d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
                d_Fa,  d_Fa2, d_Xa, d_time, p, d_Ca, changed,
                d_currentQueue, d_nextQueue, d_queueSize, d_nextQueueSize , component , componentId
                );
         ...
         // printf("queueSize:%d\n", *queueSize);
        Bfs << <*queueSize / 512 + 1, 512, 0 >> > (level, b_off, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
            d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
            d_Fa, d_Fa2, d_Xa, d_time, p, d_Ca, changed,
            d_currentQueue, d_nextQueue, d_queueSize, d_nextQueueSize, d_TimeAndP, component, componentId
            );
        CHECK(cudaGetLastError());
        CHECK(cudaDeviceSynchronize());
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    int* cc_count;
    CHECK(cudaMallocManaged(&cc_count, sizeof(int)));
    *cc_count = 0;
    bool* c_Fa, * isExist, * c_Fa2;
    int* c_Xa;
    bool* h_Xa = (bool*)malloc(MAX_Vertex * sizeof(bool));
    memset(h_Xa, 0, MAX_Vertex * sizeof(bool));
    CHECK(cudaMalloc((void**)&c_Fa, MAX_Vertex * sizeof(bool)));
    CHECK(cudaMalloc((void**)&isExist, MAX_LABEL * sizeof(bool)));
    CHECK(cudaMalloc((void**)&c_Xa, MAX_Vertex * sizeof(int)));
    CHECK(cudaMalloc((void**)&c_Fa2, MAX_Vertex * sizeof(bool)));
    CHECK(cudaMemset(c_Fa2, 0, MAX_Vertex * sizeof(bool)));
    CHECK(cudaMemset(c_Fa, 0, MAX_Vertex * sizeof(bool)));
    CHECK(cudaMemset(c_Xa, 0, MAX_Vertex * sizeof(int)));
    CHECK(cudaMemset(isExist, 0, MAX_LABEL * sizeof(bool)));
    int* c_Ca;
    std::vector<int> c_distance(MAX_Vertex, std::numeric_limits<int>::max());
    CHECK(cudaMalloc((void**)&c_Ca, MAX_Vertex * sizeof(int)));
    CHECK(cudaMemcpy(c_Ca, c_distance.data(), MAX_Vertex * sizeof(int), cudaMemcpyHostToDevice));
    int* c_time;
    CHECK(cudaMalloc((void**)&c_time, MAX_Vertex * sizeof(int)));
    CHECK(cudaMemset(c_time, 0, MAX_Vertex * sizeof(int)));
    int* c_p;
    CHECK(cudaMalloc((void**)&c_p, MAX_Vertex * sizeof(int)));
    CHECK(cudaMemset(c_p, 0, MAX_Vertex * sizeof(int)));
    //设置源节点
    int firstVertex = 1;
    int s_c_Ca = 0;
    bool  s_c_Fa = true;
    int s_c_time = -1;
    //  CHECK(cudaMemcpy(&c_Fa[firstVertex], &s_c_Fa, sizeof(bool), cudaMemcpyHostToDevice));

    int* c_currentQueue, * c_nextQueue;
    int* c_queueSize, * c_nextQueueSize;
    CHECK(cudaMalloc((void**)&c_currentQueue, MAX_Vertex * sizeof(int)));
    CHECK(cudaMalloc((void**)&c_nextQueue, MAX_Vertex * sizeof(int)));
    CHECK(cudaMalloc((void**)&c_queueSize, sizeof(int)));
    CHECK(cudaMalloc((void**)&c_nextQueueSize, sizeof(int)));
    int QueueSize = 1;
    int NextQueueSize = 0;
    CHECK(cudaMemcpy(c_queueSize, &QueueSize, sizeof(int), cudaMemcpyHostToDevice));
    CHECK(cudaMemcpy(c_nextQueueSize, &NextQueueSize, sizeof(int), cudaMemcpyHostToDevice));
    ...
    int* component;
    CHECK(cudaMalloc((void**)&component, MAX_Vertex * sizeof(int)));
    CHECK(cudaMemset(component, 0, sizeof(int)));
    ...
    getExistNode << < grid_size, 512, 0 >> > (GAMMA, ZETA_3, Graph, g_pos, d_lowerBitsVector, d_selectUpper, isExist, cc_count);

    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());
    bool* h_isExist = (bool*)malloc(MAX_LABEL * sizeof(bool));

    CHECK(cudaMemcpy(h_isExist, isExist, MAX_LABEL * sizeof(bool), cudaMemcpyDeviceToHost));
    ...
    *cc_count = 0;
    int pre_count=0;
    int flag=0;
    int limit = 0.8 * MAX_LABEL;
    for (int v = CONNECTED_COMPONENTS_VERTEX_BEGIN; v < MAX_LABEL; ++v) {
        if (*cc_count > limit) break;
        if (!h_Xa[v] && h_isExist[v]) {
            CHECK(cudaMemcpy(&c_Fa[v], &s_c_Fa, sizeof(bool), cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(&c_Ca[v], &s_c_Ca, sizeof(int), cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(&c_time[v], &s_c_time, sizeof(int), cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(&c_currentQueue[0], &v, sizeof(int), cudaMemcpyHostToDevice));
            tmp.time = 0;
            h_timeAndP[v] = toULL(tmp);
            CHECK(cudaMemcpy(d_timeAndP, h_timeAndP.data(), MAX_Vertex * sizeof(uint64_t), cudaMemcpyHostToDevice));
            tmp.p = 0;
            tmp.time = c_distance[v];
            h_timeAndP[v] = toULL(tmp);

           //  printf("第%d次   ", componentId);
           //  printf("falg：%d  ", flag);
           //  printf("节点：%d次\n", *cc_count);
            ...
            QueueSize = 1;
            CHECK(cudaMemcpy(&c_currentQueue[0], &v, sizeof(int), cudaMemcpyHostToDevice));
            run_BFS(v, component, componentId, i, 0, 0, dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
                d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper, c_Fa, c_Fa2, c_Xa, c_time, c_p, c_Ca, changed,
                &QueueSize,
                &NextQueueSize, c_currentQueue, c_nextQueue, c_queueSize, c_nextQueueSize, d_timeAndP, cc_count

            );
            componentId++;
            if(*cc_count - pre_count < 5) flag++;
        else flag=0;
        if(flag>50)break;
        }

        CHECK(cudaMemcpy(h_Xa, c_Fa, MAX_Vertex * sizeof(bool), cudaMemcpyDeviceToHost));
        CHECK(cudaDeviceSynchronize());
    ...
    long CC_duration = std::chrono::duration_cast<std::chrono::milliseconds>(CC_end - CC_start).count();
