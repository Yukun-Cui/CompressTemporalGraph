...
__device__ void GC_get_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc,
    uint32_t* vplist, uint32_t* randlist, uint64_t* inworklist, uint64_t* outworklist, uint32_t* inworklist_size, uint32_t* outworklist_size, unsigned color)
{
   ...
   // printf("here berfore travel\n");
    ...
    int previous_v = -1;
    //for GC
    uint32_t local_rand = randlist[x];
    ...
    while (index < d) {
        ...
        if (result != previous_v && result <= MAX_Vertex && result >= ACTIVE_VERTEX_BEGIN) {
            ...
            if (((randlist[result] > local_rand) || (randlist[result] == local_rand && result < x)) && vplist[result] >= color)
            {
                found_larger = true;
                break;
    ...
    if (!found_larger) {
        ...
        uint32_t idx = atomicAdd(outworklist_size, 1);
        outworklist[idx] = x;
    }
...
__device__ void GC_travel(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
  ...
  //if(i==1)
   ...
   //  if (n == 31) printf("position:%lu", *s);
    ...
    if (ref > 0) {
        ...
        GC_get_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, vplist, randlist, inworklist, outworklist, inworklist_size, outworklist_size, color);
...
__global__ void initialize_randlist(uint32_t* randlist) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < MAX_Vertex) {
        // Initialize curand
        curandState state;
        curand_init(SEED, tid, 0, &state);
        randlist[tid] = curand(&state) % 256; // Assign random priority
    }
}

__global__ void GC_kernel(
    ...
    uint32_t* vplist, uint32_t* randlist, uint64_t* inworklist, uint64_t* outworklist, uint32_t* inworklist_size, uint32_t* outworklist_size, unsigned color) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= *inworklist_size) return;

    uint64_t vid = inworklist[tid];
    if (vid < ACTIVE_VERTEX_BEGIN) return;
    if (vplist[vid] == MY_INFINITY) {
        uint32_t local_rand = randlist[vid];

        GC_travel(off, i, offsetGraph, offsetTimestamps,
            dv2, GAMMA, ZETA_3, vid, arr, s, d_Timestamps, t_pos,
            d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
            vplist, randlist, inworklist, outworklist, inworklist_size, outworklist_size, color);
...
void graphColoring(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps,
    bool* dv2, int* GAMMA, int* ZETA_3, uint8_t* Graph, uint64_t* g_pos, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper) {
    uint32_t* d_vpl = nullptr, * d_rand = nullptr;
    uint64_t* d_inworklist = nullptr, * d_outworklist = nullptr;
    uint32_t* d_inworklist_size = nullptr, * d_outworklist_size = nullptr;


    unsigned int num_thread_per_block = 512;
    unsigned int num_block = (unsigned int)ceil(MAX_Vertex / (double)num_thread_per_block);

    cudaMalloc((void**)&d_vpl, MAX_Vertex * sizeof(uint32_t));
    cudaMalloc((void**)&d_rand, MAX_Vertex * sizeof(uint32_t));
    cudaMalloc((void**)&d_inworklist, MAX_Vertex * sizeof(uint64_t));
    cudaMalloc((void**)&d_outworklist, MAX_Vertex * sizeof(uint64_t));
    cudaMalloc((void**)&d_inworklist_size, sizeof(uint32_t));
    cudaMalloc((void**)&d_outworklist_size, sizeof(uint32_t));



    // Initialize worklist and random priorities
    uint64_t* tmplist = new uint64_t[MAX_Vertex];
    for (unsigned i = 0; i < MAX_Vertex; i++) {
        tmplist[i] = i;
    }
    uint32_t vertex_cnt = MAX_Vertex;
    cudaMemcpy(d_inworklist, tmplist, MAX_Vertex * sizeof(uint64_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_inworklist_size, &vertex_cnt, sizeof(uint32_t), cudaMemcpyHostToDevice);
    delete[] tmplist;

    // Initialize the vertex properties on the GPU
    cudaMemset(d_vpl, 0xFF, MAX_Vertex * sizeof(uint32_t));


    // Initialize the random list on the GPU
    initialize_randlist << <num_block, num_thread_per_block >> > (d_rand);
    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());

    int curr = 0;
    uint32_t wl_size = MAX_Vertex;
    while (wl_size != 0 && curr < 512) {
        GC_kernel << <num_block, num_thread_per_block >> > (

            off, i, offsetGraph, offsetTimestamps,
            dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
            d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
            d_vpl, d_rand, d_inworklist, d_outworklist, d_inworklist_size, d_outworklist_size, curr);
        CHECK(cudaGetLastError());
        CHECK(cudaDeviceSynchronize());
        cudaMemcpy(&wl_size, d_outworklist_size, sizeof(uint32_t), cudaMemcpyDeviceToHost);
        cudaMemcpy(d_inworklist, d_outworklist, wl_size * sizeof(uint64_t), cudaMemcpyDeviceToDevice);
        cudaMemcpy(d_inworklist_size, d_outworklist_size, sizeof(uint32_t), cudaMemcpyDeviceToDevice);
        cudaMemset(d_outworklist_size, 0, sizeof(uint32_t));
        CHECK(cudaGetLastError());
        CHECK(cudaDeviceSynchronize());
        curr++;
    ...
    // cudaMemcpy(vproplist, d_vpl, vertex_cnt * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    ...
    cudaFree(d_vpl);
    cudaFree(d_rand);
    cudaFree(d_inworklist);
    cudaFree(d_outworklist);
    cudaFree(d_inworklist_size);
    cudaFree(d_outworklist_size);
}
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    //   Graph Color  ====================================================================================
    printf("Starting simple parallel Graph Color.\n");

    auto GC_start = std::chrono::steady_clock::now();
    graphColoring(off, i, 0, 0,
        dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
        d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    auto GC_end = std::chrono::steady_clock::now();
    long GC_duration = std::chrono::duration_cast<std::chrono::milliseconds>(GC_end - GC_start).count();
