...
__device__ void count_degree_dest(int dest_size, uint64_t offsetGraph, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, bool isfc, int* d_degree, uint64_t* d_avgtime, uint64_t  startTimestamp, uint64_t endTimestamp)
{
  ...
  // printf("get dest_size: %d\n", dest_size);
   ...
   // printf("here berfore travel\n");
    ...
    while (index < d) {
        ...
        if (x<0 || x>MAX_LABEL) printf("x out of range\n");
        //  if(x==112202)
        ...
        if (startTimestamp == endTimestamp == 0) {
            d_degree[x]++;
            d_avgtime[x] += (timestamp - d_avgtime[x]) / d_degree[x];
        }
        else {
...
__device__ void count_degree_travel(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps, bool* dv2, int* GAMMA, int* ZETA_3, int x, uint8_t* arr, uint64_t* s, uint8_t* d_Timestamps, uint64_t* t_pos,
    uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper, int* d_degree, uint64_t* d_avgtime, uint64_t  startTimestamp, uint64_t endTimestamp)
{
    // printf("here travel\n" );
    int x1 = x; //此线程编号 
  //  int n =4;
  //if(i==1)
   ...
   //  if (n == 31) printf("position:%lu", *s);
    ...
    if (ref > 0) {
        ...
        count_degree_dest(d, offsetGraph, dv2, GAMMA, ZETA_3, x1, arr, &ref_s, d_Timestamps, &tmp_t, d_lowerBitsVector, d_selectUpper, 1, d_degree, d_avgtime, startTimestamp, endTimestamp);
...
__global__ void count_degree(int* d_degree, uint64_t* d_avgtime, uint64_t startTimestamp, uint64_t endTimestamp,
...
{
    uint64_t tid = ACTIVE_VERTEX_BEGIN + blockIdx.x * blockDim.x + threadIdx.x;
    if (tid > MAX_LABEL) return;

    count_degree_travel(off, i, offsetGraph, offsetTimestamps,
        dv2, GAMMA, ZETA_3, tid, Graph, g_pos, d_Timestamps, t_pos,
        d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper,
        d_degree, d_avgtime, startTimestamp, endTimestamp);


}


__global__ void compute_centre(int* d_degree, uint64_t* d_avgtime, uint64_t startTimestamp, uint64_t endTimestamp
)
{
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid > MAX_LABEL) return;

    d_degree[tid] = 0.85 * d_degree[tid] + 0.15 * d_degree[tid] * (d_avgtime[tid] - startTimestamp) / (endTimestamp - startTimestamp) + 1;


}

void degree_centr(int off, int i, uint64_t offsetGraph, uint64_t offsetTimestamps,
bool* dv2, int* GAMMA, int* ZETA_3, uint8_t* Graph, uint64_t* g_pos, uint8_t* d_Timestamps, uint64_t* t_pos,
uint8_t* d_lowerBitsVector, SimpleSelect* d_selectUpper, uint8_t* d_t_lowerBitsVector, SimpleSelect* d_t_selectUpper, uint64_t startTimestamp = 0, uint64_t endTimestamp = 0) {
int* d_degree = 0;
uint64_t* d_avgtime = 0;




// 尽可能使用多线程处理每个顶点。如果达到最大线程数，则将其分成多个块。
unsigned int num_thread_per_block = 512;

unsigned int num_block = (unsigned int)ceil(MAX_LABEL / (double)num_thread_per_block);

// 在GPU端分配内存
cudaMalloc((void**)&d_degree, (MAX_LABEL+1) * sizeof(int));



// 初始化
cudaMemset(d_degree, 0, (MAX_LABEL + 1) * sizeof(int));

cudaMalloc((void**)&d_avgtime, (MAX_LABEL + 1) * sizeof(uint64_t));



// 初始化
cudaMemset(d_avgtime, 0, (MAX_LABEL + 1) * sizeof(uint64_t));

// 核函数计算度中心性

count_degree << <num_block, num_thread_per_block >> > (d_degree, d_avgtime, startTimestamp, endTimestamp,
    off, i, offsetGraph, offsetTimestamps,
    dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
    d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
CHECK(cudaGetLastError());
CHECK(cudaDeviceSynchronize());
compute_centre << <num_block, num_thread_per_block >> > (d_degree, d_avgtime, startTimestamp, endTimestamp);
CHECK(cudaGetLastError());
CHECK(cudaDeviceSynchronize());
...
int main() {
 ...
 //  long long totalDuration = 0;
   ...
   //  CHECK(cudaMalloc((void**)&dest, 4095 * sizeof(int)));
    ...
    printf("Starting simple parallel Degree Centre.\n");

    auto DC_start = std::chrono::steady_clock::now();
    degree_centr(off, i, 0, 0,
        dv2, GAMMA, ZETA_3, Graph, g_pos, d_Timestamps, t_pos,
        d_lowerBitsVector, d_selectUpper, d_t_lowerBitsVector, d_t_selectUpper);
    auto DC_end = std::chrono::steady_clock::now();
    long DC_duration = std::chrono::duration_cast<std::chrono::milliseconds>(DC_end - DC_start).count();
